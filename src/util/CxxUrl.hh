...
class Url {
 public:
    ...
    Url &operator=(const std::string &url_str) {
        return str(url_str);
    }
    ...
    const KeyVal &query(size_t i) const {
        lazy_parse();
        if (i >= m_query.size()) throw std::out_of_range("Invalid Url query index (" + std::to_string(i) + ")");
        return m_query[i];
    }
    ...
    const std::string get_query(const std::string &key) {
        lazy_parse();
