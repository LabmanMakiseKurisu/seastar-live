...
/// @param[in] data audio/video element data, AAC: AAC-Frame, H.264: MP4 Stream,
/// MP3-Raw data
/// @param[in] bytes data length in byte
/// @param[in] cts composition timestamp offset
/// @param[in] dts audio/video decoding timestamp
/// @param[in] flags 1-video keyframe, other-undefined
/// @return 0-ok, other-error
typedef int (*flv_parser_handler)(
    void* param, int codec, const void* data, size_t bytes, int32_t cts,
    uint32_t dts, int flags);

/// Input FLV Audio/Video Stream
/// @param[in] type 8-audio, 9-video, 18-script (see more flv-proto.h)
/// @param[in] data flv audio/video Stream, AudioTagHeader/VideoTagHeader + A/V
/// Data
/// @param[in] bytes data length in byte
