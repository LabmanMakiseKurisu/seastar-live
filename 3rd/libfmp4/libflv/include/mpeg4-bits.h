...
#define mpeg4_bits_write_uint8(bits, v, n) \
    mpeg4_bits_write_n(bits, (uint64_t)v, n)
#define mpeg4_bits_write_uint16(bits, v, n) \
    mpeg4_bits_write_n(bits, (uint64_t)v, n)
#define mpeg4_bits_write_uint32(bits, v, n) \
    mpeg4_bits_write_n(bits, (uint64_t)v, n)
#define mpeg4_bits_write_uint64(bits, v, n) \
    mpeg4_bits_write_n(bits, (uint64_t)v, n)

static inline void mpeg4_bits_init(
    struct mpeg4_bits_t* bits, void* data, size_t size) {
...
static inline uint64_t mpeg4_bits_read_n(struct mpeg4_bits_t* bits, int n) {
    ...
    m = n;
    v = bits->data[bits->bits / 8] &
        (0xFFU >> (bits->bits % 8));  // remain valid value
...
static inline int mpeg4_bits_write_n(
    struct mpeg4_bits_t* bits, uint64_t v, int n) {
    int m;
    size_t i;
    ...
    bits->data[bits->bits / 8] |=
        v >> (56 + (bits->bits % 8));  // remain valid value
    v <<= 8 - (bits->bits % 8);
    n -= 8 - (int)(bits->bits % 8);
