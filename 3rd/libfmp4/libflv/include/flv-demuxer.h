...
/// @param[in] data audio/video element data, AAC: ADTS + AAC-Frame, H.264:
/// startcode + NALU, MP3-Raw data
...
typedef int (*flv_demuxer_handler)(
    void* param, int codec, const void* data, size_t bytes, uint32_t pts,
...
int flv_demuxer_input(
    flv_demuxer_t* demuxer, int type, const void* data, size_t bytes,
    uint32_t timestamp);
