...
*/
int mpeg4_hevc_decoder_configuration_record_load(
    const uint8_t* data, size_t bytes, struct mpeg4_hevc_t* hevc) {
    uint8_t nalutype;
    ...
    hevc->general_constraint_indicator_flags =
        (uint32_t)((data[6] << 24) | (data[7] << 16) | (data[8] << 8) | data[9]);
    hevc->general_constraint_indicator_flags =
    ...
    for (i = 0; i < numOfArrays; i++) {
        ...
        for (j = 0; j < n; j++) {
            if (hevc->numOfArrays >=
                sizeof(hevc->nalu) / sizeof(hevc->nalu[0])) {
                assert(0);
            ...
            k = (p[0] << 8) | p[1];
            if (p + 2 + k > data + bytes ||
                dst + k > hevc->data + sizeof(hevc->data)) {
                assert(0);
                return -1;
            }

            assert((nalutype & 0x3F) == ((p[2] >> 1) & 0x3F));
            hevc->nalu[hevc->numOfArrays].array_completeness =
                (nalutype >> 7) & 0x01;
...
int mpeg4_hevc_decoder_configuration_record_save(
    const struct mpeg4_hevc_t* hevc, uint8_t* data, size_t bytes) {
    uint16_t n;
    uint8_t i, j, k;
    uint8_t *ptr, *end;
    uint8_t* p = data;
    uint8_t array_completeness = 1;
    const uint8_t nalu[] = { H265_VPS, H265_SPS, H265_PPS, H265_PREFIX_SEI,
                             H265_SUFFIX_SEI };
    ...
    data[1] = ((hevc->general_profile_space & 0x03) << 6) |
              ((hevc->general_tier_flag & 0x01) << 5) |
    ...
    data[21] = (hevc->constantFrameRate << 6) |
               ((hevc->numTemporalLayers & 0x07) << 3) |
               ((hevc->temporalIdNested & 0x01) << 2) |
    ...
    for (k = i = 0; i < sizeof(nalu) / sizeof(nalu[0]); i++) {
        ...
        for (n = j = 0; j < hevc->numOfArrays; j++) {
            ...
            assert(
                hevc->nalu[i].data + hevc->nalu[j].bytes <=
                hevc->data + sizeof(hevc->data));
...
int mpeg4_hevc_codecs(
    const struct mpeg4_hevc_t* hevc, char* codecs, size_t bytes) {
    ...
    // profile_compatibility reserve bit order, dot + 32-bit hex number (max 9
    // chars) tier and level, e.g. '.H120' (max 5 chars) up to 6 constraint
    // bytes, bytes are dot-separated and hex-encoded.
    const char* tier = "LH";
    ...
    x = (x >> 16) | (x << 16);
    return snprintf(
        codecs, bytes, "hvc1.%s%u.%x.%c%u",
        space[hevc->general_profile_space % 4],
        (unsigned int)hevc->general_profile_idc, (unsigned int)x,
        tier[hevc->general_tier_flag % 2],
        (unsigned int)hevc->general_level_idc);
}
...
void mpeg4_hevc_test(void) {
    const unsigned char src[] = {
        ...
        0x00, 0xb4, 0xf0, 0x00, 0xfc, 0xfd, 0xf8, 0xf8, 0x00, 0x00, 0x0f,
        0x03, 0xa0, 0x00, 0x01, 0x00, 0x18, 0x40, 0x01, 0x0c, 0x01, 0xff,
        0xff, 0x01, 0x60, 0x00, 0x00, 0x03, 0x00, 0x80, 0x00, 0x00, 0x03,
        0x00, 0x00, 0x03, 0x00, 0xb4, 0x9d, 0xc0, 0x90, 0xa1, 0x00, 0x01,
        0x00, 0x29, 0x42, 0x01, 0x01, 0x01, 0x60, 0x00, 0x00, 0x03, 0x00,
        0x80, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0xb4, 0xa0, 0x01,
        0xe0, 0x20, 0x02, 0x1c, 0x59, 0x67, 0x79, 0x24, 0x6d, 0xae, 0x01,
        0x00, 0x00, 0x03, 0x03, 0xe8, 0x00, 0x00, 0x5d, 0xc0, 0x08, 0xa2,
    ...
    const unsigned char nalu[] = {
        ...
        0xb4, 0x9d, 0xc0, 0x90, 0x00, 0x00, 0x00, 0x01, 0x42, 0x01, 0x01, 0x01,
        0x60, 0x00, 0x00, 0x03, 0x00, 0x80, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03,
        0x00, 0xb4, 0xa0, 0x01, 0xe0, 0x20, 0x02, 0x1c, 0x59, 0x67, 0x79, 0x24,
        0x6d, 0xae, 0x01, 0x00, 0x00, 0x03, 0x03, 0xe8, 0x00, 0x00, 0x5d, 0xc0,
        0x08, 0x00, 0x00, 0x00, 0x01, 0x44, 0x01, 0xc1, 0x73, 0xd1, 0x89
    };
    ...
    assert(
        sizeof(src) ==
        mpeg4_hevc_decoder_configuration_record_load(src, sizeof(src), &hevc));
    ...
    assert(
        sizeof(src) == mpeg4_hevc_decoder_configuration_record_save(
                           &hevc, data, sizeof(data)));
