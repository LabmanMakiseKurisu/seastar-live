...
static int flv_read_header(struct flv_reader_t* flv) {
    ...
    for (n = (int)(h.offset - FLV_HEADER_SIZE); n > 0 && n < 4096;
         n -= sizeof(data))
        flv->read(
            flv->param, data, n >= sizeof(data) ? sizeof(data) : n);  // skip
...
void* flv_reader_create2(
    int (*read)(void* param, void* buf, int len), void* param) {
    struct flv_reader_t* flv;
...
int flv_reader_read(
    void* p, int* tagtype, uint32_t* timestamp, size_t* taglen, void* buffer,
    size_t bytes) {
    int r;
    uint32_t sz;
    ...
    if (FLV_TAG_HEADER_SIZE !=
        flv_tag_header_read(&tag, header, FLV_TAG_HEADER_SIZE))
        return -1;
