...
int mpeg4_aac_adts_pce_load(
    const uint8_t* data, size_t bytes, struct mpeg4_aac_t* aac);
int mpeg4_aac_adts_pce_save(
    uint8_t* data, size_t bytes, const struct mpeg4_aac_t* aac);
int mpeg4_aac_audio_specific_config_load2(
    const uint8_t* data, size_t bytes, struct mpeg4_aac_t* aac);
int mpeg4_aac_audio_specific_config_save2(
    const struct mpeg4_aac_t* aac, uint8_t* data, size_t bytes);
...
int mpeg4_aac_adts_load(
    ...
    aac->profile = ((data[2] >> 6) & 0x03) +
                   1;  // 2 bits: the MPEG-2 Audio Object Type add 1
    aac->sampling_frequency_index =
        (data[2] >> 2) &
        0x0F;  // 4 bits: MPEG-4 Sampling Frequency Index (15 is forbidden)
    aac->channel_configuration =
        ((data[2] & 0x01) << 2) |
        ((data[3] >> 6) & 0x03);  // 3 bits: MPEG-4 Channel Configuration
    assert(aac->profile > 0 && aac->profile < 31);
    assert(aac->channel_configuration >= 0 && aac->channel_configuration <= 7);
    assert(
        aac->sampling_frequency_index >= 0 &&
        aac->sampling_frequency_index <= 0xc);
    aac->channels = aac->channel_configuration;
    aac->sampling_frequency =
        mpeg4_aac_audio_frequency_to(aac->sampling_frequency_index);
...
/// @return >=0-adts header length, <0-error
int mpeg4_aac_adts_save(
    const struct mpeg4_aac_t* aac, size_t payload, uint8_t* data,
    size_t bytes) {
    ...
    data[1] = 0xF0 /* 12-syncword */ | (ID << 3) /*1-ID*/ |
              (0x00 << 2) /*2-layer*/ | 0x01 /*1-protection_absent*/;
    data[2] = ((aac->profile - 1) << 6) |
              ((aac->sampling_frequency_index & 0x0F) << 2) |
...
int mpeg4_aac_adts_frame_length(const uint8_t* data, size_t bytes) {
    ...
    if (bytes < 7)
        return -1;
    // assert(0xFF == data[0] && 0xF0 == (data[1] & 0xF0)); /* syncword */
    len = ((uint16_t)(data[3] & 0x03) << 11) | ((uint16_t)data[4] << 3) |
          ((uint16_t)(data[5] >> 5) & 0x07);
    return len;
}
...
*/
/// @return >=0-adts header length, <0-error
int mpeg4_aac_audio_specific_config_load(
    const uint8_t* data, size_t bytes, struct mpeg4_aac_t* aac) {
    if (bytes < 2)
        return -1;
    ...
    if (0 == aac->channel_configuration || 5 == aac->profile ||
        29 == aac->profile || 31 == aac->profile ||
        0x0F == aac->sampling_frequency_index)
        return mpeg4_aac_audio_specific_config_load2(data, bytes, aac);
    return 2;
}

// ISO-14496-3 AudioSpecificConfig
int mpeg4_aac_audio_specific_config_save(
    const struct mpeg4_aac_t* aac, uint8_t* data, size_t bytes) {
    ...
    assert(
        ...
        aac->sampling_frequency_index <= 0xc);
    data[0] =
        (aac->profile << 3) | ((aac->sampling_frequency_index >> 1) & 0x07);
    data[1] = ((aac->sampling_frequency_index & 0x01) << 7) |
              ((channel_configuration & 0xF) << 3) |
              (0 << 2) /* frame length-1024 samples*/ |
              (0 << 1) /* don't depend on core */ | 0 /* not extension */;
...
int mpeg4_aac_stream_mux_config_load(
    const uint8_t* data, size_t bytes, struct mpeg4_aac_t* aac) {
    if (bytes < 6)
        return -1;
    assert(0 == (0x80 & data[0]));  // audioMuxVersion: 0
    // [0] 0-audioMuxVersion(1), 1-allStreamsSameTimeFraming(1),
    // 0-numSubFrames(6)
    aac->profile = ((data[1] & 0x01) << 4) |
                   (data[2] >> 4);  // 0-numProgram(4), 0-numLayer(3), 1-ASC(1)
...
// ISO/IEC 14496-3:2009(E) Table 1.42 - Syntax of StreamMuxConfig() (p83)
int mpeg4_aac_stream_mux_config_save(
    const struct mpeg4_aac_t* aac, uint8_t* data, size_t bytes) {
    if (bytes < 6)
        return -1;
    ...
    assert(
        aac->sampling_frequency_index >= 0 &&
        aac->sampling_frequency_index <= 0xc);
    data[0] = 0x40;  // 0-audioMuxVersion(1), 1-allStreamsSameTimeFraming(1),
                     // 0-numSubFrames(6)
    // data[1] = 0x00 | ((aac->profile >> 4) & 0x01); // 0-numProgram(4),
    // 0-numLayer(3) data[2] = ((aac->profile & 0x0F) << 4) |
    // (aac->sampling_frequency_index & 0x0F);
    data[1] = 0x00;
    data[2] = 0x20 | (aac->sampling_frequency_index & 0x0F);  // AAC_LC profile
    data[3] = ((aac->channel_configuration & 0x0F) << 4) |
              0;     // 0-GASpecificConfig(3), 0-frameLengthType(1)
    data[4] = 0x3F;  // 0-frameLengthType(2), 111111-latmBufferFullness(6)
    data[5] = 0xC0;  // 11-latmBufferFullness(2), 0-otherDataPresent,
                     // 0-crcCheckPresent
    return 6;
}

// ISO/IEC 14496-3:2009(E)  Table 1.14 - audioProfileLevelIndication values
// (p51)
...
static const int s_frequency[] = { 96000, 88200, 64000, 48000, 44100,
...
int mpeg4_aac_audio_frequency_from(int frequence) {
    int i = 0;
    while (i < ARRAYOF(s_frequency) && s_frequency[i] != frequence)
        i++;
...
void mpeg4_aac_test(void) {
    ...
    //	const unsigned char ascsbr[] = { 0x13, 0x10, 0x56, 0xe5, 0x9d, 0x48,
    // 0x00 };
    ...
    assert(
        sizeof(ascsbr) ==
        mpeg4_aac_audio_specific_config_load(ascsbr, sizeof(ascsbr), &aac));
    assert(
        2 == aac.profile && 7 == aac.sampling_frequency_index &&
        2 == aac.channel_configuration);
    // assert(sizeof(ascsbr) == mpeg4_aac_audio_specific_config_save(&aac, data,
    // sizeof(data))); assert(0 == memcmp(ascsbr, data, sizeof(ascsbr)));

    assert(
        sizeof(asc) ==
        mpeg4_aac_audio_specific_config_load(asc, sizeof(asc), &aac));
    assert(
        2 == aac.profile && 7 == aac.sampling_frequency_index &&
        1 == aac.channel_configuration);
    assert(
        sizeof(asc) ==
        mpeg4_aac_audio_specific_config_save(&aac, data, sizeof(data)));
    ...
    assert(
        aac.sampling_frequency_index == mpeg4_aac_audio_frequency_from(22050));

    // assert(sizeof(ascsbr) == mpeg4_aac_audio_specific_config_load(ascsbr,
    // sizeof(ascsbr), &aac)); assert(2 == aac.profile && 6 ==
    // aac.sampling_frequency_index && 1 == aac.channel_configuration);
}
#endif
