...
// There may be up to 16 such elements per raw data block, each one must have a
// unique element_instance_tag PCEs must come before all other syntactic
// elements in a raw_data_block().
/*
...
*/
static inline uint64_t mpeg4_bits_copy(
    struct mpeg4_bits_t* dst, struct mpeg4_bits_t* src, int n) {
    uint64_t v;
...
static int mpeg4_aac_pce_load(
    struct mpeg4_bits_t* bits, struct mpeg4_aac_t* aac,
...
GASpecificConfig (samplingFrequencyIndex, channelConfiguration, audioObjectType)
{
    ...
    if (extensionFlag) {
        ...
        if (audioObjectType == 17 || audioObjectType == 19 || audioObjectType ==
20 || audioObjectType == 23) {
...
*/
static int mpeg4_aac_ga_specific_config_load(
    struct mpeg4_bits_t* bits, struct mpeg4_aac_t* aac) {
    int extensionFlag;
    struct mpeg4_bits_t pce;
    ...
    if (0 == aac->channel_configuration) {
        ...
        aac->npce =
            mpeg4_aac_pce_load(bits, aac, &pce);  // update channel count
    }
    ...
    if (extensionFlag) {
        ...
        if (17 == aac->profile || 19 == aac->profile || 20 == aac->profile ||
            23 == aac->profile) {
...
static inline uint8_t mpeg4_aac_get_audio_object_type(
...
static inline uint8_t mpeg4_aac_get_sampling_frequency(
    struct mpeg4_bits_t* bits) {
...
int mpeg4_aac_audio_specific_config_load2(
    ...
    if (5 == aac->profile || 29 == aac->profile) {
        ...
        extensionSamplingFrequencyIndex =
            mpeg4_aac_get_sampling_frequency(&bits);
        aac->profile = mpeg4_aac_get_audio_object_type(&bits);
        if (22 == aac->profile)
