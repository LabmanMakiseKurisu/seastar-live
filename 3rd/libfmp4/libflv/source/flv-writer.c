...
static int flv_write_header(struct flv_writer_t* flv) {
    ...
    flv_tag_size_write(
        header + FLV_HEADER_SIZE, 4, 0);  // PreviousTagSize0(Always 0)
    return sizeof(header) == flv->write(flv->param, header, sizeof(header))
               ? 0
               : -1;
}
...
void* flv_writer_create2(
    int (*write)(void* param, const void* buf, int len), void* param) {
    struct flv_writer_t* flv;
...
int flv_writer_input(
    void* p, int type, const void* data, size_t bytes, uint32_t timestamp) {
    ...
    flv_tag_size_write(
        buf + FLV_TAG_HEADER_SIZE, 4, (uint32_t)bytes + FLV_TAG_HEADER_SIZE);

    if (FLV_TAG_HEADER_SIZE !=
            flv->write(flv->param, buf, FLV_TAG_HEADER_SIZE)  // FLV Tag Header
