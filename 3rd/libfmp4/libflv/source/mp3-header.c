...
static int s_bitrate_mpeg1[3][16] = {
    { 0 /*free*/, 32000, 64000, 96000, 128000, 160000, 192000, 224000, 256000,
      288000, 320000, 352000, 384000, 416000, 448000, -1 },
    { 0 /*free*/, 32000, 48000, 56000, 64000, 80000, 96000, 112000, 128000,
      160000, 192000, 224000, 256000, 320000, 384000, -1 },
    { 0 /*free*/, 32000, 40000, 48000, 56000, 64000, 80000, 96000, 112000,
      128000, 160000, 192000, 224000, 256000, 320000, -1 },
};

// layer-1, layer-2, layer-3
static int s_bitrate_mpeg2[3][16] = {
    { 0 /*free*/, 32000, 48000, 56000, 64000, 80000, 96000, 112000, 128000,
      144000, 160000, 176000, 192000, 224000, 256000, -1 },
    { 0 /*free*/, 8000, 16000, 24000, 32000, 40000, 48000, 56000, 64000, 80000,
...
int mp3_header_load(struct mp3_header_t* mp3, const void* data, int bytes) {
    ...
    */
    ...
    p = data;
    // 如果是ID3V1 就直接跳过 128个字节
    if (0 == memcmp("TAG", p, 3)) {
        // id3v1 固定 128   4个字符的帧头
    ...
    // ID3V2
        ...
        if (3 != p[3] /*version*/ || bytes < 10)
            return 0;
        n = (((uint32_t)p[6] & 0x7F) << 21) | (((uint32_t)p[7] & 0x7F) << 14) |
...
int mp3_header_save(const struct mp3_header_t* mp3, void* data, int bytes) {
    ...
    p[0] = 0xFF;
    p[1] =
        (uint8_t)(0xE0 | (mp3->version << 3) | (mp3->layer << 1) | mp3->protection);
    p[2] =
        (uint8_t)((mp3->bitrate_index << 4) | (mp3->sampling_frequency << 2) | 0x00 /*padding*/ | mp3->priviate);
    p[3] =
        (uint8_t)((mp3->mode << 6) | (mp3->mode_extension << 4) | (mp3->copyright << 3) | (mp3->original << 2) | mp3->emphasis);
    return 4;
}
...
int mp3_get_frequency(const struct mp3_header_t* mp3) {
    ...
    switch (mp3->version) {
        case MP3_MPEG1:
            return s_frequency_mpeg1[mp3->sampling_frequency];
        case MP3_MPEG2:
            return s_frequency_mpeg2[mp3->sampling_frequency];
        case MP3_MPEG2_5:
            return s_frequency_mpeg25[mp3->sampling_frequency];
        default:
            assert(0);
            return -1;
    }
