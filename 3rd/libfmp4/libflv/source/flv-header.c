...
int flv_header_read(struct flv_header_t* flv, const uint8_t* buf, int len) {
    if (len < FLV_HEADER_SIZE || 'F' != buf[0] || 'L' != buf[1] ||
        'V' != buf[2]) {
        assert(0);
        return -1;
    }
...
int flv_tag_header_read(
    struct flv_tag_header_t* tag, const uint8_t* buf, int len) {
    ...
    assert(
        FLV_TYPE_VIDEO == tag->type || FLV_TYPE_AUDIO == tag->type ||
...
int flv_audio_tag_header_read(
    struct flv_audio_tag_header_t* audio, const uint8_t* buf, int len) {
    assert(len > 0);
    ...
    if (FLV_AUDIO_AAC == audio->codecid) {
        ...
        assert(
            FLV_SEQUENCE_HEADER == audio->avpacket ||
            FLV_AVPACKET == audio->avpacket);
        return 2;
    } else {
        return 1;
    }
...
int flv_video_tag_header_read(
    ...
    if (FLV_VIDEO_H264 == video->codecid || FLV_VIDEO_H265 == video->codecid ||
        ...
        video->cts =
            (video->cts + 0xFF800000) ^ 0xFF800000;  // signed 24-integer
        if (video->cts >= 500) {
            printf("CTS=%d\n", video->cts);
        }
        assert(
            FLV_SEQUENCE_HEADER == video->avpacket ||
            FLV_AVPACKET == video->avpacket ||
...
int flv_header_write(int audio, int video, uint8_t* buf, int len) {
    ...
    buf[4] =
        ((audio ? 1 : 0) << 2) | (video ? 1 : 0);  // Type flags (audio & video)
    be_write_uint32(buf + 5, FLV_HEADER_SIZE);     // Data offset
    return FLV_HEADER_SIZE;
}

int flv_tag_header_write(
    const struct flv_tag_header_t* tag, uint8_t* buf, int len) {
    if (len < FLV_TAG_HEADER_SIZE) {
        assert(0);
        return -1;
    }
...
int flv_audio_tag_header_write(
    const struct flv_audio_tag_header_t* audio, uint8_t* buf, int len) {
    ...
    if (FLV_AUDIO_AAC == audio->codecid) {
        assert(
            FLV_SEQUENCE_HEADER == audio->avpacket ||
            FLV_AVPACKET == audio->avpacket);
        buf[0] = (FLV_AUDIO_AAC /* <<4 */) /* SoundFormat */ |
                 (3 << 2) /* 44k-SoundRate */ | (1 << 1) /* 16-bit samples */ |
                 1 /* Stereo sound */;
...
int flv_video_tag_header_write(
    const struct flv_video_tag_header_t* video, uint8_t* buf, int len) {
    if (len < 1)
        return -1;

    buf[0] = (video->keyframe << 4) /*FrameType*/ |
             (video->codecid & 0x0F) /*CodecID*/;
