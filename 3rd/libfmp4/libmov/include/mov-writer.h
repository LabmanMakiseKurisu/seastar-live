...
/// @param[in] flags mov flags, such as: MOV_FLAG_FASTSTART, see more
/// @mov-format.h
...
int mov_writer_add_audio(
    mov_writer_t* mov, uint8_t object, int channel_count, int bits_per_sample,
    int sample_rate, const void* extra_data, size_t extra_data_size);
int mov_writer_add_video(
    mov_writer_t* mov, uint8_t object, int width, int height,
    const void* extra_data, size_t extra_data_size);
int mov_writer_add_subtitle(
    mov_writer_t* mov, uint8_t object, const void* extra_data,
...
int mov_writer_write(
    mov_writer_t* mov, int track, const void* data, size_t bytes, int64_t pts,
