...
*/
static int mov_read_base_descr(
    struct mov_t* mov, int bytes, int* tag, int* len) {
    int i;
    uint32_t c;
...
static uint32_t mov_write_base_descr(
    const struct mov_t* mov, uint8_t tag, uint32_t len) {
...
tag=DecoderConfigDescrTag { bit(8) objectTypeIndication; bit(6) streamType;
    bit(1) upStream;
...
static int mp4_read_decoder_config_descriptor(struct mov_t* mov, int len) {
    ...
    entry->object_type_indication =
        (uint8_t)mov_buffer_r8(&mov->io); /* objectTypeIndication */
    entry->stream_type =
        (uint8_t)mov_buffer_r8(&mov->io) >> 2;             /* stream type */
    ...
    mov->track->bitrate = bit_rate;
    return mp4_read_tag(
        mov, (uint64_t)len - 13);  // mp4_read_decoder_specific_info
}

static int mp4_write_decoder_config_descriptor(const struct mov_t* mov) {
    const struct mov_sample_entry_t* entry = mov->track->stsd.current;
    int size =
        13 + (entry->extra_data_size > 0 ? entry->extra_data_size + 5 : 0);
    ...
    mov_buffer_w32(&mov->io, mov->track->bitrate); /* max bit-rate */
    mov_buffer_w32(&mov->io, mov->track->bitrate); /* avg bit-rate */
...
static int mp4_read_sl_config_descriptor(struct mov_t* mov) {
    ...
    // useTimeStampsFlag
    if (0 == (flags & 0x04)) {
        // uint64_t startDecodingTimeStamp = 0; //
        // file_reader_rb8(timeStampLength / 8) uint64_t
        // startCompositionTimeStamp = 0; // file_reader_rb8(timeStampLength /
        // 8)
    }
    return mov_buffer_error(&mov->io);
}
...
static size_t mp4_write_es_descriptor(const struct mov_t* mov) {
    ...
    size += 5 + 13 +
            (entry->extra_data_size > 0
                 ? entry->extra_data_size + 5
                 : 0);  // mp4_write_decoder_config_descriptor
