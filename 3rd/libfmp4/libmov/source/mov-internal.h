...
// Video encoding requirements 1.10: Use 'avc1', 'hvc1', or 'dvh1' rather than
// 'avc3', 'hev1', or 'dvhe'
#define MOV_H264 \
    MOV_TAG('a', 'v', 'c', '1')               // H.264 ISO/IEC 14496-15:2010(E) 5.3.4 AVC
                                              // Video Stream Definition (18)
...
#define MOV_EAC3 MOV_TAG('e', 'c', '-', '3')
#define MOV_DTS MOV_TAG('d', 't', 's', 'c')  // DTS-HD

// drm
// video
#define MOV_ENCV MOV_TAG('e', 'n', 'c', 'v')
#define MOV_FRMA MOV_TAG('f', 'r', 'm', 'a')
#define MOV_SCHM MOV_TAG('s', 'c', 'h', 'm')
#define MOV_SCHI MOV_TAG('s', 'c', 'h', 'i')
#define MOV_TECH MOV_TAG('t', 'e', 'n', 'c')
#define MOV_PSSH MOV_TAG('p', 's', 's', 'h')
// audio
#define MOV_ENCA MOV_TAG('e', 'n', 'c', 'a')

enum
{
    DRM_ENC_NONE = 0x00,
    DRM_ENC_AES_128 = 0x01,
    DRM_ENC_CENC = 0x02,
    DRM_ENC_CBC1 = 0x03,
    DRM_ENC_CENS = 0x04,
    DRM_ENC_CBCS = 0x05,
...
enum
{
    ...
    MOV_BRAND_DBY1 = MOV_TAG('d', 'b', 'y', '1'),
};
...
struct mov_sample_t {
    ...
    void* data;
    uint64_t offset;  // is a 32 or 64 bit integer that gives the offset of the
    ...
    uint32_t duration;
    // for senc
    mov_senc_info_t* senc_info;
    uint32_t subsample_count;
    size_t subsample_offset;  // subsample_capacity
#endif
...
struct mov_t {
    ...
    int is_dovi;
    struct dovi_config_t dv_config;
    struct eac3_config_t da_config;

    char* v_extra_data;
    int64_t v_extra_size;
    int64_t v_extra_cap;

    char* a_extra_data;
    int64_t a_extra_size;
    int64_t a_extra_cap;
    // uint8_t video_extra_data[1024];
    // int64_t v_extra_size;
    // uint8_t audio_extra_data[1024];
    // int64_t a_extra_size;
};
...
int mov_read_dvvc(struct mov_t* mov, const struct mov_box_t* box);
...
size_t mov_write_dvvc(const struct mov_t* mov);
...
size_t mov_write_senc(const struct mov_t* mov, uint32_t from, uint32_t count);
...
int mov_add_audio(
    struct mov_track_t* track, const struct mov_mvhd_t* mvhd, uint32_t timescale, uint8_t object, int channel_count,
    int bits_per_sample, int sample_rate, const void* extra_data, size_t extra_data_size);
int mov_add_video(
    struct mov_track_t* track, const struct mov_mvhd_t* mvhd, uint32_t timescale, uint8_t object, int width, int height,
    const void* extra_data, size_t extra_data_size);
int mov_add_subtitle(
    struct mov_track_t* track, const struct mov_mvhd_t* mvhd, uint32_t timescale, uint8_t object,
    const void* extra_data, size_t extra_data_size);
