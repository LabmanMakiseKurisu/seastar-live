...
void bitstream_init(
    bitstream_t* stream, const unsigned char* h264, size_t bytes) {
...
int bitstream_next_bit(bitstream_t* stream) {
    ...
    return (stream->h264[stream->offsetBytes] >>
            (BIT_NUM - 1 - stream->offsetBits)) &
           0x01;
}
...
int bitstream_read_bit(bitstream_t* stream) {
    ...
    bit = (stream->h264[stream->offsetBytes] >>
           (BIT_NUM - 1 - stream->offsetBits)) &
          0x01;
...
int bitstream_read_me(
    bitstream_t* stream, int chroma_format_idc, int coded_block_pattern) {
    static int intra[48] = { 0,  16, 1,  2,  4,  8,  32, 3,  5,  10, 12, 15,
                             47, 7,  11, 13, 14, 6,  9,  31, 35, 37, 42, 44,
                             33, 34, 36, 40, 39, 43, 45, 46, 17, 18, 20, 24,
                             19, 21, 26, 28, 23, 27, 29, 30, 22, 25, 38, 41 };
    static int intra_4x4_8x8[48] = { 47, 31, 15, 0,  23, 27, 29, 30, 7,  11,
                                     13, 14, 39, 43, 45, 46, 16, 3,  5,  10,
                                     12, 19, 21, 26, 28, 35, 37, 42, 44, 1,
                                     2,  4,  8,  17, 18, 20, 24, 6,  9,  22,
                                     25, 32, 33, 34, 36, 40, 38, 41 };

    static int chroma_intra[16] = { 15, 0,  7, 11, 13, 14, 3, 5,
                                    10, 12, 1, 2,  4,  8,  6, 9 };
    static int chroma_intra_4x4_8x8[16] = { 0,  1,  2, 4,  8,  3,  5, 10,
