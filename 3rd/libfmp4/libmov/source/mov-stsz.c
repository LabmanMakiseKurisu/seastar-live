...
int mov_read_stsz(struct mov_t* mov, const struct mov_box_t* box) {
    ...
    assert(
        0 == track->sample_count &&
        NULL == track->samples);  // duplicated STSZ atom
    if (track->sample_count < sample_count) {
        void* p = realloc(
            track->samples, sizeof(struct mov_sample_t) * (sample_count + 1));
        if (NULL == p)
            return ENOMEM;
        track->samples = (struct mov_sample_t*)p;
        memset(
            track->samples, 0,
            sizeof(struct mov_sample_t) * (sample_count + 1));
    }
    track->sample_count = sample_count;

    if (0 == sample_size) {
        for (i = 0; i < sample_count; i++)
            track->samples[i].bytes =
                mov_buffer_r32(&mov->io);  // uint32_t entry_size
    } else {
        for (i = 0; i < sample_count; i++)
...
size_t mov_write_stsz(const struct mov_t* mov) {
    ...
    size = 12 /* full box */ + 8 +
           (i < track->sample_count ? 4 * track->sample_count : 0);
    ...
    if (i < track->sample_count) {
        ...
        mov_buffer_w32(
            &mov->io, track->sample_count < 1 ? 0 : track->samples[0].bytes);
        mov_buffer_w32(&mov->io, track->sample_count);
    }
