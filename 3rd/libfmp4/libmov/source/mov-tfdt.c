...
int mov_read_tfdt(struct mov_t* mov, const struct mov_box_t* box) {
    ...
    if (1 == version)
        mov->track->tfdt_dts =
            mov_buffer_r64(&mov->io); /* baseMediaDecodeTime */
    else
        mov->track->tfdt_dts =
            mov_buffer_r32(&mov->io); /* baseMediaDecodeTime */
...
size_t mov_write_tfdt(const struct mov_t* mov) {
    ...
    if (1 == version)
        ...
        mov_buffer_w32(
            &mov->io, (uint32_t)baseMediaDecodeTime); /* baseMediaDecodeTime */
