...
int mov_read_smdm(struct mov_t* mov, const struct mov_box_t* box) {
    ...
    mov_buffer_r16(&mov->io);  // primaryRChromaticity_x, 0.16 fixed-point Red X
    ...
    mov_buffer_r32(
        &mov->io);  // luminanceMax, 24.8 fixed point Maximum luminance,
                    // represented in candelas per square meter (cd/m²)
...
int mov_read_coll(struct mov_t* mov, const struct mov_box_t* box) {
    ...
    mov_buffer_r16(&mov->io);  // maxCLL, Maximum Content Light Level as
                               // specified in CEA-861.3, Appendix A.
    mov_buffer_r16(&mov->io);  // maxFALL, Maximum Frame-Average Light Level as
                               // specified in CEA-861.3, Appendix A.
    return mov_buffer_error(&mov->io);
}
