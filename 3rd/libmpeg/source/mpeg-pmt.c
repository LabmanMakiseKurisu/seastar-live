...
// audio information: Systems 2.4.4.8 Program map table(p68)
...
static int pmt_read_descriptor(
    struct pes_t* stream, const uint8_t* data, uint16_t bytes) {
    uint8_t tag;
    uint8_t len;
    uint8_t channels;
    ...
    while (bytes > 2) {
        ...
        switch (tag) {
            ...
            case 0x81:  // AC-3 descriptor
            case 0xCC:  // EAC-3 decriptor
                stream->esinfo = (uint8_t*)calloc(len, 1);
                if (stream->esinfo) {
                    memcpy(stream->esinfo, data + 2, len);
            ...
            case 0x28:
                stream->codecid = PSI_STREAM_H264;
                break;

            // hevc video descriptor
            case 0x38:
                stream->codecid = PSI_STREAM_H265;
                break;
            ...
            case 0xB0:  // DOVI video stream descriptor
                ...
                // parse dv
                // int dv_version_major = data[2];
                // int dv_version_minor = data[3];
                // int dv_profile = data[4] >> 1;
                // int dv_level = ((data[4] & 0x01) << 5) | (data[5] >> 3);
                // int rpu_flag = data[5] >> 2 & 0x01;
                // int el_flag = data[5] >> 1 & 0x01;
                // int bl_flag = data[5] & 0x01;
                break;

            case 0x7f:  // DVB-Service Information: 6.1 Descriptor
                        // identification and location (p38)
                // 2.6.90 Extension descriptor
                if (PSI_STREAM_AUDIO_OPUS == stream->codecid &&
                    OPUS_EXTENSION_DESCRIPTOR_TAG ==
                        data[2])  // User defined (provisional Opus)
                {
                    channels = data[3];
                    stream->esinfo =
                        (uint8_t*)calloc(1, sizeof(opus_default_extradata));
                    if (stream->esinfo) {
                        stream->esinfo_len = sizeof(opus_default_extradata);
                        memcpy(
                            stream->esinfo, opus_default_extradata,
                            stream->esinfo_len);
                        stream->esinfo[9] = channels ? channels : 2;
                        stream->esinfo[18] = channels ? (channels > 2 ? 1 : 0)
                                                      : /* Dual Mono */ 255;
                        stream->esinfo[19] = opus_stream_cnt[channels];
                        stream->esinfo[20] = opus_coupled_stream_cnt[channels];
                        memcpy(
                            stream->esinfo + 21,
                            opus_channel_map[(channels ? channels : 2) - 1],
                            channels ? channels : 2);
                    }
                }
        }
...
static int pmt_write_descriptor(
    const struct pes_t* stream, uint8_t* data, int bytes) {
    uint8_t* p;

    p = data;
    if (PSI_STREAM_AUDIO_OPUS == stream->codecid &&
        bytes > 2 + 4 /*fourcc*/ + 4 /*DVI OPUS*/) {
        *p++ = 0x05;  // 2.6.8 Registration descriptor(p94)
        *p++ = 4;
        memcpy(p, "Opus", 4);
        p += 4;

        *p++ = 0x7f;  // DVB-Service Information: 6.1 Descriptor identification
                      // and location (p38)
        *p++ = 2;
        *p++ = OPUS_EXTENSION_DESCRIPTOR_TAG;
        *p++ = stream->esinfo_len > 8 ? stream->esinfo[9]
                                      : 2 /*0xFF*/;  // default 2-channels
    }
...
size_t pmt_read(struct pmt_t* pmt, const uint8_t* data, size_t bytes) {
    ...
    //(unsigned int)data[3], (unsigned int)data[4], (unsigned int)data[5],
    //(unsigned int)data[6],(unsigned int)data[7],(unsigned
    // int)data[8],(unsigned int)data[9],(unsigned int)data[10],(unsigned
    // int)data[11]);
    ...
    for (i = 12 + program_info_length;
         i + 5 <= section_length + 3 - 4 /*CRC32*/;
         i += len + 5)  // 9: follow section_length item
    {
        pid = ((data[i + 1] & 0x1F) << 8) | data[i + 2];
        len = ((data[i + 3] & 0x0F) << 8) | data[i + 4];
        //        printf("PMT: pn: %0x, pid: %0x, codec: %0x, eslen: %d\n",
        //        (unsigned int)pmt->pn, (unsigned int)pid, (unsigned
        //        int)data[i], (unsigned int)len);

        if (i + len + 5 > section_length + 3 - 4 /*CRC32*/)
            break;
        assert(
            pmt->stream_count <=
            sizeof(pmt->streams) / sizeof(pmt->streams[0]));
...
size_t pmt_write(const struct pmt_t* pmt, uint8_t* data) {
    ...
    for (i = 0; i < pmt->stream_count &&
                p - data < 1021 - 4 - 5 - pmt->streams[i].esinfo_len;
         i++) {
        // stream_type
        *p = (uint8_t)(PSI_STREAM_AUDIO_OPUS == pmt->streams[i].codecid)
                 ? PSI_STREAM_PRIVATE_DATA
                 : pmt->streams[i].codecid;
        ...
        // if(PSI_STREAM_AUDIO_OPUS == pmt->streams[i].codecid ||
        // (pmt->streams[i].esinfo_len > 0 && pmt->streams[i].esinfo))
        {
            // assert(pmt->streams[i].esinfo);
            // memcpy(p, pmt->streams[i].esinfo, pmt->streams[i].esinfo_len);
            // p += pmt->streams[i].esinfo_len;
            len = pmt_write_descriptor(
                &pmt->streams[i], p + 5, 1021 - (int)(p + 5 - data));
        }
