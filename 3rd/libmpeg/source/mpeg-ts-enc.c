...
#define PES_HEADER_LEN \
    6  // 3-bytes packet_start_code_prefix + 1-byte stream_id + 2-bytes
       // PES_packet_length
...
static int mpeg_ts_write_section_header(
    const mpeg_ts_enc_context_t* ts, int pid, unsigned int* cc,
    ...
    *cc = (*cc + 1) % 16;  // update continuity_counter
    ...
    //		data[4] = TS_PACKET_SIZE - len - 5 - 1; // 4B-Header + 1B-pointer +
    // 1B-self 		if(data[4] > 0)
    //		{
    //			// adaptation
    //			data[5] = 0; // no flag
...
static int ts_write_pes(
    mpeg_ts_enc_context_t* tsctx, const struct pmt_t* pmt, struct pes_t* stream,
    ...
    while (0 == r && bytes > 0) {
        ...
        if (start && stream->data_alignment_indicator &&
            PTS_NO_VALUE != stream->pts) {
            // In the PCR_PID the random_access_indicator may only be set to '1'
            // in a transport stream packet containing the PCR fields.
            data[3] |= 0x20;  // +AF
            data[5] |=
                AF_FLAG_RANDOM_ACCESS_INDICATOR;  // +random_access_indicator
        }
        ...
        if (data[3] & 0x20) {
            ...
            header = data + TS_HEADER_LEN + 1 +
                     data[4];  // 4-TS + 1-AF-Len + AF-Payload
        } else {
        ...
        if (start) {
            data[1] |=
                TS_PAYLOAD_UNIT_START_INDICATOR;  // payload_unit_start_indicator

            p += pes_write_header(
                stream, header, TS_PACKET_SIZE - (header - data));

            if (PSI_STREAM_H264 == stream->codecid &&
                !tsctx->h264_h265_with_aud) {
                // 2.14 Carriage of Rec. ITU-T H.264 | ISO/IEC 14496-10 video
                // Each AVC access unit shall contain an access unit delimiter
                // NAL Unit
                ...
                PSI_STREAM_H265 == stream->codecid &&
                !tsctx->h264_h265_with_aud) {
                // 2.17 Carriage of HEVC
                // Each HEVC access unit shall contain an access unit delimiter
                // NAL unit.
                nbo_w32(p, 0x00000001);
                p[4] = 0x46;  // 35-AUD_NUT
                p[5] = 0x01;
            ...
            // payload consists of bytes from a video elementary stream
            // contained in transport stream packets
            if ((p - header - PES_HEADER_LEN) + bytes > 0xFFFF)
                nbo_w16(
                    header + 4, 0);  // 2.4.3.7 PES packet => PES_packet_length
            else
                nbo_w16(
                    header + 4,
                    (uint16_t)((p - header - PES_HEADER_LEN) + bytes));
        }
        ...
        if (len + bytes < TS_PACKET_SIZE) {
            ...
            if (p - header > 0) {
                assert(start);
                memmove(
                    data + (TS_PACKET_SIZE - bytes - (p - header)), header,
                    p - header);
            }
            ...
            if (data[3] & 0x20)  // has AF?
            {
                ...
                memset(
                    data + TS_HEADER_LEN + 1 + data[4], 0xFF,
                    TS_PACKET_SIZE - (len + bytes));
                data[4] += (uint8_t)(TS_PACKET_SIZE - (len + bytes));
            } else {
                assert(len == (size_t)(p - header) + TS_HEADER_LEN);
                data[3] |= 0x20;  // +AF
                data[4] =
                    (uint8_t)(TS_PACKET_SIZE - (len + bytes) - 1 /*AF length*/);
                if (data[4] > 0)
                    data[5] = 0;  // no flag
                if (data[4] > 1)
...
static struct pes_t* mpeg_ts_find(
    mpeg_ts_enc_context_t* ts, int pid, struct pmt_t** pmt) {
    size_t i, j;
    struct pes_t* stream;
...
int mpeg_ts_write(
    void* ts, int pid, int flags, int64_t pts, int64_t dts, const void* data,
    size_t bytes) {
    int r = 0;
    size_t i, n;
    ...
    if (0 == ++tsctx->pat_cycle % PAT_CYCLE || 0 == tsctx->pat_period ||
        ...
        if (0 == tsctx->sdt_period) {
            ...
            r = mpeg_ts_write_section_header(
                ts, TS_PID_SDT, &tsctx->pat.cc /*fixme*/, tsctx->payload, n);
            if (0 != r)
                return r;
        }
        ...
        r = mpeg_ts_write_section_header(
            ts, TS_PID_PAT, &tsctx->pat.cc, tsctx->payload,
            n);  // PID = 0x00 program association table
        if (0 != r)
            return r;
        ...
        for (i = 0; i < tsctx->pat.pmt_count; i++) {
            ...
            r = mpeg_ts_write_section_header(
                ts, tsctx->pat.pmts[i].pid, &tsctx->pat.pmts[i].cc,
                tsctx->payload, n);
            if (0 != r)
                return r;
        }
    ...
    for (i = 0; i < tsctx->pat.pmt_count; i++) {
        ...
        if (i + 1 < tsctx->pat.pmt_count)
            memmove(
                ...
                (tsctx->pat.pmt_count - i - 1) * sizeof(tsctx->pat.pmts[0]));
        tsctx->pat.pmt_count--;
        mpeg_ts_reset(ts);  // update PAT/PMT
        return 0;
    }
...
static int mpeg_ts_pmt_add_stream(
    mpeg_ts_enc_context_t* ts, struct pmt_t* pmt, int codecid,
...
static int mpeg_ts_pmt_update_stream(
    mpeg_ts_enc_context_t* ts, struct pmt_t* pmt, int codecid,
    const void* extra_data, size_t extra_data_size) {
    struct pes_t* stream = NULL;
    if (!ts || !pmt ||
    ...
    for (int i = 0; i < pmt->stream_count; i++) {
        stream = &pmt->streams[i];
        if (stream->codecid != codecid) 
            continue;
...
int mpeg_ts_add_stream(
    ...
    return mpeg_ts_pmt_add_stream(
        tsctx, pmt, codecid, extra_data, extra_data_size);
}

int mpeg_ts_update_stream(
    void* ts, int codecid, const void* extra_data, size_t extra_data_size) {
    struct pmt_t* pmt = NULL;
    mpeg_ts_enc_context_t* tsctx;
    ...
    return mpeg_ts_pmt_update_stream(
        tsctx, pmt, codecid, extra_data, extra_data_size);
}

int mpeg_ts_add_program_stream(
    void* ts, uint16_t pn, int codecid, const void* extra_data,
