...
mpeg_muxer_t *mpeg_muxer_create(
    int is_ps, const mpeg_muxer_func_t *func, void *param) {
    ...
    if (is_ps) {
        ...
        struct mpeg_ts_func_t ts_func = { on_mpeg_ts_alloc, on_mpeg_ts_free,
                                          on_mpeg_ts_write };
        mpeg->u.ts.func = *func;
...
int mpeg_muxer_add_stream(
    mpeg_muxer_t *muxer, int codecid, const void *extradata,
    size_t extradata_size) {
    assert(muxer);
    if (muxer->is_ps) {
        return ps_muxer_add_stream(
            muxer->u.ps, codecid, extradata, extradata_size);
    }
    return mpeg_ts_add_stream(
        muxer->u.ts.ctx, codecid, extradata, extradata_size);
}

int mpeg_muxer_input(
    mpeg_muxer_t *muxer, int stream, int flags, int64_t pts, int64_t dts,
    const void *data, size_t bytes) {
    assert(muxer);
    if (muxer->is_ps) {
        return ps_muxer_input(
            muxer->u.ps, stream, flags, pts, dts, data, bytes);
    }
...
int mpeg_muxer_add_program(
    mpeg_muxer_t *muxer, uint16_t pn, const void *info, int bytes) {
    assert(muxer);
    if (muxer->is_ps) {
        return -1;
    }
...
int mpeg_muxer_add_program_stream(
    mpeg_muxer_t *muxer, uint16_t pn, int codecid, const void *extra_data,
    size_t extra_data_size) {
    assert(muxer);
    if (muxer->is_ps) {
        return -1;
    }
    return mpeg_ts_add_program_stream(
        muxer->u.ts.ctx, pn, codecid, extra_data, extra_data_size);
}