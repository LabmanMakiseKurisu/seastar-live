...
int ps_muxer_input(
    struct ps_muxer_t* ps, int streamid, int flags, int64_t pts, int64_t dts,
    ...
    sz = bytes + MAX_PES_HEADER +
         (bytes / MAX_PES_PACKET + 1) *
             64;  // 64 = 0x000001 + stream_id + PES_packet_length + other
    ...
    // fix ps_pkt_parse_system -> ps_track_fill with default mp1/2 audio
    // codec(without psm)
    ...
    while (bytes > 0) {
        ...
        if (first) {
            if (PSI_STREAM_H264 == stream->codecid && !ps->h264_h265_with_aud) {
                ...
                PSI_STREAM_H265 == stream->codecid && !ps->h264_h265_with_aud) {
        ...
        //		i = 0; // clear value, the next pes packet don't need
        // pack_header
...
struct ps_muxer_t* ps_muxer_create(
    const struct ps_muxer_func_t* func, void* param) {
    ...
    ps->system.system_audio_lock_flag =
        0;  // all audio stream sampling rate is constant
    ps->system.system_video_lock_flag =
        0;  // all video stream frequency is constant
...
int ps_muxer_add_stream(
    struct ps_muxer_t* ps, int codecid, const void* extradata, size_t bytes) {
    struct psm_t* psm;
    struct pes_t* pes;

    assert(bytes < 512);
    if (!ps || ps->psm.stream_count >=
                   sizeof(ps->psm.streams) / sizeof(ps->psm.streams[0])) {
        assert(0);
        return -1;
    }
    ...
    if (mpeg_stream_type_video(codecid)) {
        ...
        /* FIXME -- VCD uses 46, SVCD uses 230, ffmpeg has 230 with a note that
         * it is small */
        ps->system.streams[ps->system.stream_count].buffer_size_bound =
            400 /* 8191-13 bits max value */;
