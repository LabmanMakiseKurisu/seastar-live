...
// audio information: Systems 2.4.4.3 Program association table(p65)
...
struct pmt_t* pat_alloc_pmt(struct pat_t* pat) {
    ...
    if (NULL == pat->pmts) {
        ...
        pat->pmt_capacity =
            sizeof(pat->pmt_default) / sizeof(pat->pmt_default[0]);
    }
    ...
    if (pat->pmt_count >= pat->pmt_capacity) {
        ...
        ptr = realloc(
            pat->pmts == pat->pmt_default ? NULL : pat->pmts,
            sizeof(pat->pmts[0]) * n);
        if (!ptr)
            return NULL;
...
size_t pat_read(struct pat_t* pat, const uint8_t* data, size_t bytes) {
    ...
    for (i = 8; i + 4 <= section_length + 8 - 5 - 4;
         i += 4)  // 4:CRC, 5:follow section_length item
    {
        pn = (data[i] << 8) | data[i + 1];
        pid = ((data[i + 2] & 0x1F) << 8) | data[i + 3];
        //        printf("PAT: pn: %0x, pid: %0x\n", (unsigned int)pn, (unsigned
        //        int)pid);
...
size_t pat_write(const struct pat_t* pat, uint8_t* data) {
    ...
    len =
        pat->pmt_count * 4 + 5 + 4;  // 5 bytes remain header and 4 bytes crc32
