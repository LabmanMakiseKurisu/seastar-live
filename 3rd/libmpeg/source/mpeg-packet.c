...
static int mpeg_packet_append(
    struct packet_t* pkt, const void* data, size_t size) {
    void* ptr;

    // fix: pkt->size + size bits wrap
    if (pkt->size + size > MPEG_PACKET_PAYLOAD_MAX_SIZE ||
        pkt->size + size < pkt->size)
        return -EINVAL;
...
static int mpeg_packet_h264_h265_filter(
    uint16_t program, uint16_t stream, struct packet_t* pkt,
    const uint8_t* data, size_t size, pes_packet_handler handler, void* param) {
    int i;
    size_t off;
    size_t leading;
    ...
    for (off = i = 0; off < size; off += i + 1) {
        ...
        // assert(0 == i - leading);
        if (PSI_STREAM_H264 == pkt->codecid
                ? 9 == (data[off + i] & 0x1f)
                : 35 == ((data[off + i] >> 1) & 0x3f))
            continue;
    ...
    // TODO: check size > 0 ???
    return handler(
        param, program, stream, pkt->codecid, pkt->flags, pkt->pts, pkt->dts,
        data + off + i, size - off - i);
}

static int mpeg_packet_h264_h265(
    struct packet_t* pkt, const struct pes_t* pes, size_t size,
    pes_packet_handler handler, void* param) {
    int r, n;
    ...
    p = pkt->size < size + 5 ? pkt->data
                             : end - size - 5;  // start from trailing nalu
    find = PSI_STREAM_H264 == pes->codecid ? mpeg_h264_find_new_access_unit
                                           : mpeg_h265_find_new_access_unit;
    ...
    while (n >= 0) {
        ...
        p += n;
        r = mpeg_packet_h264_h265_filter(
            pes->pn, pes->pid, pkt, data, p - data, handler, param);
        if (0 != r)
            return r;
...
int pes_packet(
    struct packet_t* pkt, const struct pes_t* pes, const void* data,
    size_t size, int start, pes_packet_handler handler, void* param) {
    int r;
    ...
    if (PSI_STREAM_H264 == pes->codecid || PSI_STREAM_H265 == pes->codecid) {
        ...
        if (pkt->size > 0 && (pkt->dts != pes->dts || start)) {
            ...
            r = handler(
                param, pes->pn, pes->pid, pkt->codecid, pkt->flags, pkt->pts,
                pkt->dts, pkt->data, pkt->size);
        ...
        if (pes->len > 0 && pes->pkt.size >= pes->len) {
            assert(
                pes->pkt.size == pes->len ||
                (pkt->flags & MPEG_FLAG_PACKET_CORRUPT));  // packet lost
            r = handler(
                param, pes->pn, pes->pid, pkt->codecid, pkt->flags, pkt->pts,
                pkt->dts, pes->pkt.data, pes->len);
            pkt->size = 0;  // new packet start
        }
    }
