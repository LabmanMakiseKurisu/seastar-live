...
static uint32_t adaptation_filed_read(
    struct ts_adaptation_field_t* adp, const uint8_t* data, size_t bytes) {
    ...
    if (adp->adaptation_field_length > 0) {
        ...
        if (adp->PCR_flag) {
            adp->program_clock_reference_base =
                ((uint64_t)data[i] << 25) | ((uint64_t)data[i + 1] << 17) |
                ((uint64_t)data[i + 2] << 9) | ((uint64_t)data[i + 3] << 1) |
                ((data[i + 4] >> 7) & 0x01);
            adp->program_clock_reference_extension =
                ((data[i + 4] & 0x01) << 8) | data[i + 5];
        ...
        if (adp->OPCR_flag) {
            adp->original_program_clock_reference_base =
                ...
                ((uint64_t)data[i + 2] << 9) | ((uint64_t)data[i + 3] << 1) |
                ((data[i + 4] >> 7) & 0x01);
            adp->original_program_clock_reference_extension =
                ((data[i + 4] & 0x01) << 1) | data[i + 5];
        ...
        if (adp->adaptation_field_extension_flag) {
            ...
            if (adp->ltw_flag) {
                ...
                //				uint16_t ltw_offset = ((data[i] & 0x7F) << 8) |
                // data[i+1];
                i += 2;
            }

            if (adp->piecewise_rate_flag) {
                //				uint32_t piecewise_rate = ((data[i] & 0x3F) <<
                //16) | (data[i+1] << 8) | data[i+2];
                i += 3;
            }

            if (adp->seamless_splice_flag) {
                //				uint8_t Splice_type = (data[i] >> 4) & 0x0F;
                //				uint32_t DTS_next_AU = (((data[i] >> 1) & 0x07)
                //<< 30) | (data[i+1] << 22) | (((data[i+2] >> 1) & 0x7F) << 15)
    ...
    for (i = 0; i < ts->pat.pmt_count; i++) {
        for (j = 0; j < ts->pat.pmts[i].stream_count; j++) {
            ...
            if (PSI_STREAM_H264 == pes->codecid) {
                ...
                pes_packet(
                    &pes->pkt, pes, aud, sizeof(aud), 0, ts->onpacket,
                    ts->param);
...
int ts_demuxer_input(
    struct ts_demuxer_t* ts, const uint8_t* data, size_t bytes) {
    int r = 0;
    uint32_t i, j, k;
    uint32_t PID;
    unsigned int count;
    ...
    //	printf("-----------------------------------------------\n");
    //	printf("PID[%u]: Error: %u, Start:%u, Priority:%u, Scrambler:%u, AF: %u,
    // CC: %u\n", PID, pkhd.transport_error_indicator,
    // pkhd.payload_unit_start_indicator, pkhd.transport_priority,
    // pkhd.transport_scrambling_control, pkhd.adaptation_field_control,
    // pkhd.continuity_counter);

    i = 4;
    if (0x02 & pkhd.adaptation_field_control) {
        i += adaptation_filed_read(&pkhd.adaptation, data + 4, bytes - 4);

        if (pkhd.adaptation.adaptation_field_length > 0 &&
            pkhd.adaptation.PCR_flag) {
            // int64_t t;
            // t = pkhd.adaptation.program_clock_reference_base / 90L; // ms;
            // printf("pcr: %02d:%02d:%02d.%03d - %" PRId64 "/%u\n", (int)(t /
            // 3600000), (int)(t % 3600000)/60000, (int)((t/1000) % 60), (int)(t
            // % 1000), pkhd.adaptation.program_clock_reference_base,
            // pkhd.adaptation.program_clock_reference_extension);
        }
    }
            ...
            for (j = 0; j < ts->pat.pmt_count; j++) {
                if (PID == ts->pat.pmts[j].pid) {
                    ...
                    for (k = 0; k < ts->pat.pmts[j].stream_count; k++) {
                        ...
                        pes->flags |= ((ts->pat.pmts[j].streams[k].cc + 1) %
                                       16) != pkhd.continuity_counter
                        ...
                        if (pkhd.payload_unit_start_indicator) {
                            ...
                            pes->flags = (pes->flags & MPEG_FLAG_PACKET_CORRUPT)
                                             ? MPEG_FLAG_PACKET_LOST
                                             : 0;
                            pes->flags |= pes->data_alignment_indicator
                                              ? MPEG_FLAG_IDR_FRAME
                                              : 0;
                        ...
                        r = pes_packet(
                            &pes->pkt, pes, data + i, bytes - i,
                            pkhd.payload_unit_start_indicator, ts->onpacket,
                            ts->param);
                        pes->have_pes_header =
                            (r || (0 == pes->pkt.size && pes->len > 0))
                                ? 0
                                : 1;  // packet completed
                        break;        // find stream
                    }
...
static inline int mpeg_ts_is_idr_first_packet(const void* packet, int bytes) {
    ...
    return (payload_unit_start_indicator &&
            pkhd.adaptation.random_access_indicator)
               ? 1
               : 0;
}

struct ts_demuxer_t* ts_demuxer_create(
    ts_demuxer_onpacket onpacket, void* param) {
...
int ts_demuxer_getservice(
    struct ts_demuxer_t* ts, int program, char* provider, int nprovider,
    char* name, int nname) {
    struct pmt_t* pmt;
...
void ts_demuxer_set_notify(
    struct ts_demuxer_t* ts, struct ts_demuxer_notify_t* notify, void* param) {
    ts->notify_param = param;
    memcpy(&ts->notify, notify, sizeof(ts->notify));
}

static void ts_demuxer_notify(
    struct ts_demuxer_t* ts, const struct pmt_t* pmt) {
    unsigned int i;
    ...
    for (i = 0; i < pmt->stream_count; i++) {
        pes = &pmt->streams[i];
        ts->notify.onstream(
            ts->notify_param, pes->pid, pes->codecid, pes->esinfo,
            pes->esinfo_len, i + 1 >= pmt->stream_count ? 1 : 0, pes->is_dovi);
    }
}
