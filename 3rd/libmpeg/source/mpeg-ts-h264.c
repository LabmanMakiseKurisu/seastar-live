...
static int mpeg_h264_find_access_unit_delimiter(
    const uint8_t* p, size_t bytes, size_t* leading) {
    int i;
    size_t off;
...
int mpeg_h264_find_new_access_unit(
    const uint8_t* data, size_t bytes, int* vcl) {
    int n;
    size_t leading;
    uint8_t nal_type;
    const uint8_t *p, *end;
...
int mpeg_h26x_verify(const uint8_t* data, size_t bytes, int* codec) {
    ...
    for (p = data; p && p < end && count < sizeof(h26x[0]) / sizeof(h26x[0][0]);
         p += n) {
        n = mpeg_h264_find_nalu(p, end - p, &leading);
        if (n < 0 || p + n + 1 > end)
            break;
