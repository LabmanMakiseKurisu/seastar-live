...
size_t system_header_read(
    struct ps_system_header_t *h, const uint8_t *data, size_t bytes) {
    size_t i, j;
    size_t len;
    ...
    h->rate_bound =
        ((data[6] & 0x7F) << 15) | (data[7] << 7) | ((data[8] >> 1) & 0x7F);
    ...
    i = 12;
    for (j = 0; (data[i] & 0x80) == 0x80 &&
                j < sizeof(h->streams) / sizeof(h->streams[0]) && i < bytes;
         j++) {
        ...
        if (h->streams[j].stream_id == PES_SID_EXTENSION)  // '10110111'
        {
            ...
            h->streams[j].stream_id =
                (h->streams[j].stream_id << 7) | (data[i + 1] & 0x7F);
...
size_t system_header_write(const struct ps_system_header_t *h, uint8_t *data) {
    ...
    data[9] = ((h->audio_bound & 0x3F) << 2) | ((h->fixed_flag & 0x01) << 1) |
              (h->CSPS_flag & 0x01);

    // 1-system_audio_lock_flag + 1-system_video_lock_flag + 1-maker +
    // 5-video_bound
    data[10] = 0x20 | ((h->system_audio_lock_flag & 0x01) << 7) |
               ((h->system_video_lock_flag & 0x01) << 6) |
               (h->video_bound & 0x1F);
    ...
    for (j = 0; j < h->stream_count; j++) {
        ...
        // '11xxxxxx xxxxxxxx'
        data[i++] = 0xC0 | ((h->streams[j].buffer_bound_scale & 0x01) << 5) |
                    ((h->streams[j].buffer_size_bound >> 8) & 0x1F);
