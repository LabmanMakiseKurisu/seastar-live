...
// audio information: Systems 2.5.3.1 Program stream(p74)
...
static int ps_demuxer_onpes(
    void* param, int program, int stream, int codecid, int flags, int64_t pts,
    ...
    return ps->onpacket(
        ps->param, stream, codecid, flags, pts, dts, data, bytes);
}
...
static struct pes_t* psm_fetch(struct psm_t* psm, uint8_t sid) {
    ...
    if (psm->stream_count < sizeof(psm->streams) / sizeof(psm->streams[0])) {
        ...
        if (0xE0 <= sid && sid <= 0xEF) {
            psm->streams[psm->stream_count].codecid =
                PSI_STREAM_RESERVED;  // unknown
...
static int pes_packet_read(
    struct ps_demuxer_t* ps, const uint8_t* data, size_t bytes) {
    int r;
    size_t n;
    size_t i = 0;
    size_t j = 0;
    ...
    for (i = 0; i + 5 < bytes && 0x00 == data[i] && 0x00 == data[i + 1] &&
                0x01 == data[i + 2] && PES_SID_END != data[i + 3] &&
                PES_SID_START != data[i + 3];
         i += pes_packet_length + 6) {
        pes_packet_length = (data[i + 4] << 8) | data[i + 5];
#if defined(MPEG_ZERO_PAYLOAD_LENGTH)
        // fix H.264/H.265 ps payload zero-length
        if (0 == pes_packet_length && 0xE0 <= data[i + 3] &&
            data[i + 3] <= 0xEF) {
            pes_packet_length =
                ps_demuxer_find_pes_start(data + i + 6, bytes - i - 6);
        }
#endif
        ...
        switch (data[i + 3]) {
            ...
            default:
                ...
                if (ps->pkhd.mpeg2)
                    ...
                    j = pes_read_mpeg1_header(
                        pes, data + i, pes_packet_length + 6);

                if (j > 0) {
#if defined(MPEG_GUESS_STREAM) || defined(MPEG_H26X_VERIFY)
                    if (pes->codecid == PSI_STREAM_RESERVED &&
                        0 == mpeg_h26x_verify(
                                 data + i + j, pes_packet_length + 6 - j, &r)) {
                        // modify codecid
                        static const uint8_t sc_codecid[] = {
                            PSI_STREAM_RESERVED,
                            PSI_STREAM_H264,
                            PSI_STREAM_H265,
                            PSI_STREAM_MPEG4,
                        };
                        pes->codecid = sc_codecid
                            [(r < 0 ||
                    ...
                    if ((pes->codecid == PSI_STREAM_AUDIO_G711A ||
                         pes->codecid == PSI_STREAM_AUDIO_G711U) &&
                        pes_packet_length + 6 - j > 7 && 0xFF == data[i + j] &&
                        0xF0 == (data[i + j + 1] & 0xF0)) {
                        n = 7;
                        // calc mpeg4_aac_adts_frame_length
                        for (r = (int)(i + j);
                             (size_t)r + 7 < i + pes_packet_length + 6 &&
                             n >= 7;
                             r += n) {
                            // fix n == 0
                            n = ((uint16_t)(data[r + 3] & 0x03) << 11) |
                                ((uint16_t)data[r + 4] << 3) |
                                ((uint16_t)(data[r + 5] >> 5) & 0x07);
                        }
                        pes->codecid = (size_t)r == i + pes_packet_length + 6
                                           ? PSI_STREAM_AAC
                                           : pes->codecid;  // fix it
                    }
#endif
                    ...
                    pes->flags =
                        pes->data_alignment_indicator ? MPEG_FLAG_IDR_FRAME : 0;
                    r = pes_packet(
                        &pes->pkt, pes, data + i + j, pes_packet_length + 6 - j,
                        ps->start, ps_demuxer_onpes, ps);
...
int ps_demuxer_input(
    ...
    for (i = ps_demuxer_find_startcode(data, bytes); data && i + 3 < bytes;
         i += ps_demuxer_find_startcode(data + i, bytes - i)) {
        // fix HIK H.265: 00 00 01 BA 00 00 01 E0 ...
        if (i + 6 < bytes && 00 == data[i + 4] && 00 == data[i + 5] &&
            01 == data[i + 6]) {
            i += 4;
            continue;
        }
...
struct ps_demuxer_t* ps_demuxer_create(
    ps_demuxer_onpacket onpacket, void* param) {
    struct ps_demuxer_t* ps;
...
void ps_demuxer_set_notify(
    struct ps_demuxer_t* ps, struct ps_demuxer_notify_t* notify, void* param) {
...
static void ps_demuxer_notify(struct ps_demuxer_t* ps) {
    ...
    for (i = 0; i < ps->psm.stream_count; i++) {
        pes = &ps->psm.streams[i];
        ps->notify.onstream(
            ps->notify_param, pes->pid, pes->codecid, pes->esinfo,
            pes->esinfo_len, i + 1 >= ps->psm.stream_count ? 1 : 0);
    }
}
