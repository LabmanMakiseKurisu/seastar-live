...
// audio information: Systems 2.5.4 Program stream map(p82)
...
size_t psm_read(struct psm_t *psm, const uint8_t *data, size_t bytes) {
    ...
    if (program_stream_map_length < 3 ||
        bytes < (size_t)program_stream_map_length + 6)
    ...
    if ((size_t)program_stream_info_length + 4 + 2 /*element_stream_map_length*/
    ...
    element_stream_map_length =
        program_stream_map_length - program_stream_info_length - 10;

    i += 2;
    psm->stream_count = 0;
    for (j = i;
         j + 4 /*element_stream_info_length*/ <=
             i + element_stream_map_length &&
        ...
        psm->streams[psm->stream_count].pid =
            psm->streams[psm->stream_count].sid;  // for ts PID
        element_stream_info_length = (data[j + 2] << 8) | data[j + 3];
        if (j + 4 + element_stream_info_length > i + element_stream_map_length)
            return 0;  // TODO: error

        k = j + 4;
        if (0xFD == psm->streams[psm->stream_count].sid &&
            0 == single_extension_stream_flag) {
        ...
        while (k + 2 <= j + 4 + element_stream_info_length) {
            // descriptor()
            k += mpeg_elment_descriptor(
                data + k, j + 4 + element_stream_info_length - k);
        }
...
size_t psm_write(const struct psm_t *psm, uint8_t *data) {
    ...
    for (i = 0; i < psm->stream_count; i++) {
        ...
        // descriptor()
        memcpy(
            data + j + 2, psm->streams[i].esinfo, psm->streams[i].esinfo_len);
