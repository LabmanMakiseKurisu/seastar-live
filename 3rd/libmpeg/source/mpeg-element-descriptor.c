...
// audio information: Systems 2.6 Program and program element descriptors(p83)
...
size_t metadata_pointer_descriptor(const uint8_t* data, size_t bytes) {
    ...
    if (flags & 0x80)  // metadata_locator_record_flag
    {
        ...
        mpeg_bits_skip(
            &bits,
            desc.metadata_locator_record_length);  // metadata_locator_record_byte
    }
...
size_t metadata_descriptor(const uint8_t* data, size_t bytes) {
    ...
    if (flags & 0x10)  // DSM-CC_flag
    {
        ...
        mpeg_bits_skip(
            &bits,
            desc.service_identification_length);  // service_identification_record_byte
    }

    if (0x01 == desc.decoder_config_flags) {
        desc.decoder_config_length = mpeg_bits_read8(&bits);
        mpeg_bits_skip(
            &bits, desc.decoder_config_length);  // decoder_config_byte
    } else if (0x03 == desc.decoder_config_flags) {
        desc.dec_config_identification_record_length = mpeg_bits_read8(&bits);
        mpeg_bits_skip(
            &bits,
            desc.dec_config_identification_record_length);  // dec_config_identification_record_byte
