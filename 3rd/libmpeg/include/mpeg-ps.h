...
int ps_muxer_add_stream(
    struct ps_muxer_t* muxer, int codecid, const void* extradata, size_t bytes);
...
int ps_muxer_input(
    struct ps_muxer_t* muxer, int stream, int flags, int64_t pts, int64_t dts,
    const void* data, size_t bytes);

/// @param[in] codecid 0-unknown, other-enum EPSI_STREAM_TYPE, see more
/// @mpeg-ts-proto.h
/// @return 0-ok, other-error
typedef int (*ps_demuxer_onpacket)(
    void* param, int stream, int codecid, int flags, int64_t pts, int64_t dts,
    const void* data, size_t bytes);
...
/// ps_demuxer_input return consumed bytes, the remain data MUST save and merge
/// with next packet int n = ps_demuxer_input(demuxer, data, bytes); if(n >= 0
/// && n < bytes)
///		memcpy(NEXTBUFFER, data + n, bytes - n);
///
/// @return >=0-consume bytes, <0-error
int ps_demuxer_input(
    struct ps_demuxer_t* demuxer, const uint8_t* data, size_t bytes);
...
void ps_demuxer_set_notify(
    struct ps_demuxer_t* demuxer, struct ps_demuxer_notify_t* notify,
    void* param);
