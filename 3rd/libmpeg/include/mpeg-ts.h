...
/// @param[in] codecid PSI_STREAM_H264/PSI_STREAM_H265/PSI_STREAM_AAC, see more
/// @mpeg-ts-proto.h
/// @param[in] extradata itu h.222.0 program and program element descriptors,
/// NULL for H.264/H.265/AAC
/// @param[in] extradata_size extradata size in byte
/// @return <=0-error, >0-audio/video stream id
int mpeg_ts_add_stream(
    void* ts, int codecid, const void* extradata, size_t extradata_size);
...
/// @param[in] data H.264/H.265-AnnexB stream(include 00 00 00 01), AAC-ADTS
/// stream
/// @return 0-ok, other-error
int mpeg_ts_write(
    void* ts, int stream, int flags, int64_t pts, int64_t dts, const void* data,
    size_t bytes);
...
typedef int (*ts_demuxer_onpacket)(
...
int ts_demuxer_input(
    struct ts_demuxer_t* demuxer, const uint8_t* data, size_t bytes);
int ts_demuxer_flush(struct ts_demuxer_t* demuxer);
int ts_demuxer_getservice(
    struct ts_demuxer_t* demuxer, int program, char* provider, int nprovider,
    char* name, int nname);
...
struct ts_demuxer_notify_t {
    ...
    /// @param[in] is_dovi 0-is not dovi 1-is dovi
    void (*onstream)(
        void* param, int stream, int codecid, const void* extra, int bytes,
        int finish, int is_dovi);
};

/// Set ts notify on PMT change
void ts_demuxer_set_notify(
    struct ts_demuxer_t* demuxer, struct ts_demuxer_notify_t* notify,
    void* param);
