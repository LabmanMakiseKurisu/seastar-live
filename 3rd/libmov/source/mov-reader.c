...
// static struct mov_sample_entry_t* mov_track_stsd_find(struct mov_track_t*
// track, uint32_t sample_description_index)
//{
...
static int mov_read_trak(struct mov_t* mov, const struct mov_box_t* box) {
    ...
    if (0 == r) {
        ...
        if (mov->track->sample_count > 0) {
            ...
            mov->track->tfdt_dts =
                mov->track->samples[mov->track->sample_count - 1].dts;
        }
    }
...
static int mov_read_btrt(struct mov_t* mov, const struct mov_box_t* box) {
    ...
    mov->track->bitrate = mov_buffer_r32(&mov->io); /* avgBitrate */
    (void)box;
    return 0;
}
...
static int mov_read_moof(struct mov_t* mov, const struct mov_box_t* box) {
    ...
    // If base-data-offset-present not provided and if the default-base-is-moof
    // flag is not set, the base-data-offset for the first track in the movie
    // fragment is the position of the first byte of the enclosing Movie
    // Fragment Box, for second and subsequent track fragments, the default is
    // the end of the data defined by the preceding track fragment.
    mov->moof_offset = mov->implicit_offset =
        mov_buffer_tell(&mov->io) - 8 /*box size */;
...
static struct mov_parse_t s_mov_parse_table[] = {
    ...
    { MOV_TAG('a', 'v', 'c', 'C'), MOV_NULL,
      mov_read_avcc },  // ISO/IEC 14496-15:2010(E) avcC
    { MOV_TAG('b', 't', 'r', 't'), MOV_NULL,
      mov_read_btrt },  // ISO/IEC 14496-15:2010(E) 5.3.4.1.1 Definition
    ...
    { MOV_TAG('e', 's', 'd', 's'), MOV_NULL,
      mov_read_esds },  // ISO/IEC 14496-14:2003(E) mp4a/mp4v/mp4s
    { MOV_TAG('f', 'r', 'e', 'e'), MOV_NULL, mov_read_free },
    { MOV_TAG('f', 't', 'y', 'p'), MOV_ROOT, mov_read_ftyp },
    { MOV_TAG('g', 'm', 'i', 'n'), MOV_GMHD,
      mov_read_gmin },  // Apple QuickTime gmin
    { MOV_TAG('g', 'm', 'h', 'd'), MOV_MINF,
      mov_read_default },  // Apple QuickTime gmhd
    { MOV_TAG('h', 'd', 'l', 'r'), MOV_MDIA,
      mov_read_hdlr },  // Apple QuickTime minf also has hdlr
    { MOV_TAG('h', 'v', 'c', 'C'), MOV_NULL,
      mov_read_hvcc },  // ISO/IEC 14496-15:2014 hvcC
    { MOV_TAG('d', 'v', 'c', 'C'), MOV_NULL, mov_read_dvcc },
    { MOV_TAG('d', 'v', 'v', 'C'), MOV_NULL, mov_read_dvvc },
    ...
    //	{ MOV_TAG('n', 'm', 'h', 'd'), MOV_MINF, mov_read_default }, // ISO/IEC
    // 14496-12:2015(E) 8.4.5.2 Null Media Header Box (p45)
    ...
    //	{ MOV_TAG('s', 't', 'h', 'd'), MOV_MINF, mov_read_default }, // ISO/IEC
    // 14496-12:2015(E) 12.6.2 Subtitle media header (p185)
...
static int mov_reader_init(struct mov_t* mov) {
    ...
    for (i = 0; i < mov->track_count; i++) {
        ...
        if (0 == track->mdhd.duration && track->sample_count > 0)
            track->mdhd.duration = track->samples[track->sample_count - 1].dts -
                                   track->samples[0].dts;
        if (0 == track->tkhd.duration)
            track->tkhd.duration = track->mdhd.duration * mov->mvhd.timescale /
                                   track->mdhd.timescale;
        if (track->tkhd.duration > mov->mvhd.duration)
            mov->mvhd.duration =
                track->tkhd.duration;  // maximum track duration
    }

    return 0;
}

struct mov_reader_t* mov_reader_create(
    const struct mov_buffer_t* buffer, void* param) {
    ...
    // Files with no file-type box should be read as if they contained an FTYP
    // box with Major_brand='mp41', minor_version=0, and the single compatible
    // brand 'mp41'.
    ...
    reader->mov.v_extra_cap = 1024;
    reader->mov.a_extra_cap = 1024;
    reader->mov.v_extra_size = 0;
    reader->mov.a_extra_size = 0;

    reader->mov.v_extra_data = malloc(1024);
    reader->mov.a_extra_data = malloc(1024);
    if (reader->mov.v_extra_data == NULL || reader->mov.a_extra_data == NULL) {
...
void mov_reader_destroy(struct mov_reader_t* reader) {
    ...
    if (reader->mov.v_extra_data)
        free(reader->mov.v_extra_data);
    if (reader->mov.a_extra_data)
        free(reader->mov.a_extra_data);
    free(reader);
}
...
static struct mov_track_t* mov_reader_next(struct mov_reader_t* reader) {
    ...
    for (i = 0; i < reader->mov.track_count; i++) {
        ...
        dts = track2->samples[track2->sample_offset].dts * 1000 /
              track2->mdhd.timescale;
        // if (NULL == track || dts < best_dts)
        // if (NULL == track || track->samples[track->sample_offset].offset >
        // track2->samples[track2->sample_offset].offset)
        if (NULL == track ||
            (dts < best_dts && best_dts - dts > AV_TRACK_TIMEBASE) ||
            track2->samples[track2->sample_offset].offset <
                track->samples[track->sample_offset].offset) {
            track = track2;
            best_dts = dts;
        }
    }
...
int mov_reader_read(
    struct mov_reader_t* reader, void* buffer, size_t bytes,
    mov_reader_onread onread, void* param) {
    struct mov_track_t* track;
    ...
    onread(
        param, track->tkhd.track_ID,
        track->stsd.entries[0].object_type_indication,
        /*sample->sample_description_index-1,*/ buffer, sample->bytes,
        sample->pts * 1000 / track->mdhd.timescale,
        sample->dts * 1000 / track->mdhd.timescale, sample->flags);
    return 1;
}
...
int mov_reader_getinfo(
    struct mov_reader_t* reader, struct mov_reader_trackinfo_t* ontrack,
    void* param) {
    int i;
    uint32_t j;
    struct mov_track_t* track;
    ...
    for (i = 0; i < reader->mov.track_count; i++) {
        track = &reader->mov.tracks[i];
        for (j = 0; j < track->stsd.entry_count && j < 1 /* only the first */;
             j++) {
            ...
            switch (track->handler_type) {
                case MOV_VIDEO:
                    if (ontrack->onvideo)
                        ontrack->onvideo(
                            param, track->tkhd.track_ID,
                            entry->object_type_indication,
                            entry->u.visual.width, entry->u.visual.height,
                            entry->extra_data, entry->extra_data_size,
                            track->bitrate);
                    break;
                ...
                case MOV_AUDIO:
                    if (ontrack->onaudio)
                        ontrack->onaudio(
                            param, track->tkhd.track_ID,
                            entry->object_type_indication,
                            entry->u.audio.channelcount,
                            entry->u.audio.samplesize,
                            entry->u.audio.samplerate >> 16, entry->extra_data,
                            entry->extra_data_size, track->bitrate);
                    break;

                case MOV_SUBT:
                case MOV_TEXT:
                    if (ontrack->onsubtitle)
                        ontrack->onsubtitle(
                            param, track->tkhd.track_ID, MOV_OBJECT_TEXT,
                            entry->extra_data, entry->extra_data_size);
                    break;
...
uint64_t mov_reader_getduration(struct mov_reader_t* reader) {
    return 0 != reader->mov.mvhd.timescale
               ? reader->mov.mvhd.duration * 1000 / reader->mov.mvhd.timescale
               : 0;
}
...
static int mov_stss_seek(struct mov_track_t* track, int64_t* timestamp) {
    ...
    next =
        track->stbl.stss[mid + 1 < track->stbl.stss_count ? mid + 1 : mid] - 1;
    if (DIFF(track->samples[prev].dts, clock) <
        DIFF(track->samples[idx].dts, clock))
        idx = prev;
    if (DIFF(track->samples[next].dts, clock) <
        DIFF(track->samples[idx].dts, clock))
        idx = next;
...
static int mov_sample_seek(struct mov_track_t* track, int64_t timestamp) {
    ...
    if (DIFF(track->samples[prev].dts, timestamp) <
        DIFF(track->samples[mid].dts, timestamp))
        mid = prev;
    if (DIFF(track->samples[next].dts, timestamp) <
        DIFF(track->samples[mid].dts, timestamp))
        mid = next;
...
int64_t mov_video_extra_data(mov_reader_t* mov, char* data, int64_t size) {
    if (mov->mov.v_extra_size > size) {
        char* tmp = malloc(mov->mov.v_extra_size);
        memcpy(data, mov->mov.v_extra_data, mov->mov.a_extra_size);
        free(data);
        data = tmp;
    } else {
        memcpy(data, mov->mov.v_extra_data, mov->mov.v_extra_size);
    }
    return mov->mov.v_extra_size;
}

int64_t mov_audio_extra_data(mov_reader_t* mov, char* data, int64_t size) {
    if (mov->mov.a_extra_size > size) {
        char* tmp = malloc(mov->mov.a_extra_size);
        memcpy(data, mov->mov.a_extra_data, mov->mov.a_extra_size);
        free(data);
        data = tmp;
    } else {
        memcpy(data, mov->mov.a_extra_data, mov->mov.a_extra_size);
    }
    return mov->mov.a_extra_size;
}