...
int mov_read_tfra(struct mov_t* mov, const struct mov_box_t* box) {
    ...
    for (i = 0; i < number_of_entry; i++) {
        ...
        for (traf_number = 0, j = 0; j < ((length_size_of >> 4) & 0x03) + 1;
             j++)
            traf_number =
                (traf_number << 8) | mov_buffer_r8(&mov->io); /* traf_number */

        for (trun_number = 0, j = 0; j < ((length_size_of >> 2) & 0x03) + 1;
             j++)
            trun_number =
                (trun_number << 8) | mov_buffer_r8(&mov->io); /* trun_number */

        for (sample_number = 0, j = 0; j < (length_size_of & 0x03) + 1; j++)
            sample_number = (sample_number << 8) |
                            mov_buffer_r8(&mov->io); /* sample_number */
    }
...
size_t mov_write_tfra(const struct mov_t* mov) {
    ...
    size =
        12 /* full box */ + 12 /* base */ + track->frag_count * 19 /* index */;
