...
struct mov_writer_t* mov_writer_create(
    const struct mov_buffer_t* buffer, void* param, int flags) {
    struct mov_t* mov;
    ...
    mov->mvhd.creation_time =
        time(NULL) + 0x7C25B080;  // 1970 based -> 1904 based;
...
static int mov_writer_move(
    struct mov_t* mov, uint64_t to, uint64_t from, size_t bytes);
void mov_writer_destroy(struct mov_writer_t* writer) {
    ...
    if (writer->mdat_size + 8 <= UINT32_MAX) {
        mov_write_size(
            mov, writer->mdat_offset,
            (uint32_t)(writer->mdat_size + 8)); /* update size */
    } else {
        offset2 = mov_buffer_tell(&mov->io);
    ...
    for (i = 0; i < mov->track_count; i++) {
        ...
        // pts in ms
        track->mdhd.duration =
            track->samples[track->sample_count - 1].dts - track->samples[0].dts;
        // track->mdhd.duration = track->mdhd.duration * track->mdhd.timescale /
        // 1000;
        track->tkhd.duration =
            track->mdhd.duration * mov->mvhd.timescale / track->mdhd.timescale;
    ...
    if (MOV_FLAG_FASTSTART & mov->flags) {
        ...
        if (co64) {
            ...
            do {
                ...
                for (i = 0; i < mov->track_count; i++) {
                    co64 +=
                        mov_stco_size(&mov->tracks[i], offset2 - offset + sz);
                }
            } while (sz != co64);
        }
        ...
        mov_writer_move(
            mov, writer->mdat_offset, offset, (size_t)(offset2 - offset));
    }
...
static int mov_writer_move(
    ...
    for (i = to; i < from; i += bytes) {
        ...
        // functions are fsetpos, fseek, and rewind. When you switch from
        // writing to reading, you must use an intervening call to either fflush
...
int mov_writer_write(
    struct mov_writer_t* writer, int track, const void* data, size_t bytes,
    ...
    if (mov->track->sample_count + 1 >= mov->track->sample_offset) {
        void* ptr = realloc(
            mov->track->samples,
            sizeof(struct mov_sample_t) * (mov->track->sample_offset + 1024));
        if (NULL == ptr)
            return -ENOMEM;
...
int mov_writer_add_audio(
    struct mov_writer_t* writer, uint8_t object, int channel_count,
    int bits_per_sample, int sample_rate, const void* extra_data,
    ...
    if (0 != mov_add_audio(
                 track, &mov->mvhd, 1000, object, channel_count,
                 bits_per_sample, sample_rate, extra_data, extra_data_size))
        return -ENOMEM;
...
int mov_writer_add_video(
    struct mov_writer_t* writer, uint8_t object, int width, int height,
    ...
    if (0 != mov_add_video(
                 track, &mov->mvhd, 1000, object, width, height, extra_data,
                 extra_data_size))
        return -ENOMEM;
...
int mov_writer_add_subtitle(
    struct mov_writer_t* writer, uint8_t object, const void* extra_data,
    size_t extra_data_size) {
    struct mov_t* mov;
    struct mov_track_t* track;
    ...
    if (0 != mov_add_subtitle(
                 track, &mov->mvhd, 1000, object, extra_data, extra_data_size))
        return -ENOMEM;
