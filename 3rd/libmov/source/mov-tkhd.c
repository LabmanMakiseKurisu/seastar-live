...
size_t mov_write_tkhd(const struct mov_t* mov) {
    ...
    mov_buffer_w32(
        &mov->io, (uint32_t)tkhd->modification_time);   /* modification_time */
    ...
    // mov_buffer_w16(&mov->io, AVSTREAM_AUDIO == track->stream_type ? 0x0100 :
    // 0); /* volume */
    ...
    mov_buffer_w32(
        &mov->io, tkhd->width /*track->av.video.width * 0x10000U*/); /* width */
    mov_buffer_w32(
        &mov->io,
        tkhd->height /*track->av.video.height * 0x10000U*/); /* height */
    return 92;
}
