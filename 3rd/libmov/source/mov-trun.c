...
int mov_read_trun(struct mov_t* mov, const struct mov_box_t* box) {
    ...
    if (track->sample_count + sample_count + 1 > track->sample_offset) {
        void* p = realloc(
            track->samples, sizeof(struct mov_sample_t) *
                                (track->sample_count + 2 * sample_count + 1));
        if (NULL == p)
            return ENOMEM;
        track->samples = (struct mov_sample_t*)p;
        track->sample_offset = track->sample_count + 2 * sample_count + 1;
        memset(
            track->samples + track->sample_count, 0,
            sizeof(struct mov_sample_t) * (2 * sample_count + 1));
    }
    ...
    for (i = 0; i < sample_count; i++) {
        ...
        if (MOV_TRUN_FLAG_SAMPLE_FLAGS_PRESENT & flags)
            ...
            sample_flags =
                i ? track->tfhd.default_sample_flags : first_sample_flags;
        ...
        sample[i].flags =
            (sample_flags &
             (MOV_TREX_FLAG_SAMPLE_IS_NO_SYNC_SAMPLE | 0x01000000))
                ? 0
...
size_t mov_write_trun(
    const struct mov_t* mov, uint32_t from, uint32_t count, uint32_t moof) {
    uint32_t flags;
    uint32_t delta;
    uint64_t offset;
    uint32_t size, i;
    ...
    int default_duration =
        (MOV_TFHD_FLAG_DEFAULT_DURATION & track->tfhd.flags) != 0;
    flags |= MOV_TRUN_FLAG_FIRST_SAMPLE_FLAGS_PRESENT |
             MOV_TRUN_FLAG_SAMPLE_SIZE_PRESENT;
    if (!default_duration)
        flags |= MOV_TRUN_FLAG_SAMPLE_DURATION_PRESENT |
                 MOV_TRUN_FLAG_SAMPLE_COMPOSITION_TIME_OFFSET_PRESENT;
#else
    ...
    if (flags & MOV_TRUN_FLAG_FIRST_SAMPLE_FLAGS_PRESENT) {
        ...
        mov_buffer_w32(
            &mov->io,
            (track->samples[from].flags & MOV_AV_FLAG_KEYFREAME)
                ? MOV_TREX_FLAG_SAMPLE_DEPENDS_ON_I_PICTURE
                : (MOV_TREX_FLAG_SAMPLE_IS_NO_SYNC_SAMPLE |
                   MOV_TREX_FLAG_SAMPLE_DEPENDS_ON_NOT_I_PICTURE)); /* first_sample_flags
                                                                     */
#else
        mov_buffer_w32(
            &mov->io,
            MOV_TREX_FLAG_SAMPLE_DEPENDS_ON_I_PICTURE); /* first_sample_flags */
#endif
        size += 4;
    }
    ...
    for (i = from; i < from + count; i++) {
        ...
        if (default_duration) {
            ...
            mov_buffer_w32(&mov->io, sample->duration); /* sample_duration */
            mov_buffer_w32(&mov->io, (uint32_t)sample->bytes); /* sample_size */
            mov_buffer_w32(
                &mov->io,
                (int32_t)(sample->pts - sample->dts)); /* sample_composition_time_offset
                                                        */
            size += 12;
        }
#else
