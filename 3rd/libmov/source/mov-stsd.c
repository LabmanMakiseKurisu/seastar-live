...
static int mp4_read_extra(struct mov_t* mov, const struct mov_box_t* box) {
    ...
    mov_buffer_seek(&mov->io, p1);

    if (mov->track->handler_type == MOV_VIDEO) {
        ...
        mov_buffer_read(&mov->io, mov->v_extra_data, p2 - p1);
        mov->v_extra_size += box->size;
    } else if (mov->track->handler_type == MOV_AUDIO) {
        if (mov->a_extra_size + box->size > mov->a_extra_cap) {
            // need to recalloc
            mov->a_extra_cap *= 2;
            char* tmp = malloc(mov->a_extra_cap);
            memcpy(tmp, mov->a_extra_data, mov->a_extra_size);
            free(mov->a_extra_data);
            mov->a_extra_data = tmp;
        }
        memcpy(mov->a_extra_data, &box->size, 4);
        memcpy(mov->a_extra_data, &box->type, 4);
        mov_buffer_read(&mov->io, mov->a_extra_data, p2 - p1);
        mov->a_extra_size += box->size;
    } else {
        fprintf(stderr, "maybe subtitle track, not to supported");
    }
    ...
    // 	if (mov->track->handler_type == MOV_VIDEO)
    // 	{
    // 		assert(mov->v_extra_size <= 1024);
    // 		memcpy(mov->video_extra_data, &size, 4);
    // 		memcpy(mov->video_extra_data, &box->type, 4);
    // 		mov_buffer_read(&mov->io, mov->video_extra_data, p2 - p1);
    // 		mov->v_extra_size += size;
    // 	}
    // 	else if (mov->track->handler_type == MOV_AUDIO)
    // 	{
    // 		assert(mov->a_extra_size <= 1024);
    // 		memcpy(mov->audio_extra_data, &size, 4);
    // 		memcpy(mov->audio_extra_data, &box->type, 4);
    // 		mov_buffer_read(&mov->io, mov->audio_extra_data, p2 - p1);
    // 		mov->a_extra_size += size;
    // 	}
    // 	else {
    // 		assert(0);
    // 	}
...
*/
static int mov_read_sample_entry(
    struct mov_t* mov, struct mov_box_t* box, uint16_t* data_reference_index) {
...
static int mov_read_audio(struct mov_t* mov, struct mov_sample_entry_t* entry) {
    ...
    entry->u.audio.samplerate =
        mov_buffer_r32(&mov->io);  // { default samplerate of media}<<16;
...
static int mov_read_video(struct mov_t* mov, struct mov_sample_entry_t* entry) {
    ...
    entry->u.visual.horizresolution =
        mov_buffer_r32(&mov->io);  // 0x00480000 - 72 dpi
    entry->u.visual.vertresolution =
        mov_buffer_r32(&mov->io);  // 0x00480000 - 72 dpi
    // const unsigned int(32) reserved = 0;
...
static int mov_read_hint_sample_entry(
    struct mov_t* mov, struct mov_sample_entry_t* entry) {
    struct mov_box_t box;
    mov_read_sample_entry(mov, &box, &entry->data_reference_index);
...
static int mov_read_meta_sample_entry(
    struct mov_t* mov, struct mov_sample_entry_t* entry) {
    struct mov_box_t box;
    mov_read_sample_entry(mov, &box, &entry->data_reference_index);
    mov_buffer_skip(&mov->io, box.size - 16);
...
*/
static int mov_read_text_sample_entry(
    struct mov_t* mov, struct mov_sample_entry_t* entry) {
    struct mov_box_t box;
    mov_read_sample_entry(mov, &box, &entry->data_reference_index);
    if (MOV_TEXT == box.type) {
        ...
        // mov_buffer_r16(&mov->io);
        // mov_buffer_r64(&mov->io); /* default text box (top, left, bottom,
        // right) */ mov_buffer_r64(&mov->io); /* reserved */
...
*/
static int mov_read_subtitle_sample_entry(
    struct mov_t* mov, struct mov_sample_entry_t* entry) {
    struct mov_box_t box;
    mov_read_sample_entry(mov, &box, &entry->data_reference_index);
...
int mov_read_stsd(struct mov_t* mov, const struct mov_box_t* box) {
    ...
    if (track->stsd.entry_count < entry_count) {
        void* p = realloc(
            track->stsd.entries, sizeof(track->stsd.entries[0]) * entry_count);
        if (NULL == p)
            return ENOMEM;
...
static size_t mov_write_video(
    const struct mov_t* mov, const struct mov_sample_entry_t* entry) {
    size_t size;
    uint64_t offset;
#ifndef _BILI_FIXED_
    ...
    if (mov->is_drm) {
        mov_buffer_w32(&mov->io, MOV_ENCV);          // "encv"
    } else {
        mov_buffer_w32(&mov->io, mov->track->tag);   // "h264"  "hevc"
    }

    mov_buffer_w32(&mov->io, 0);                /* Reserved */
    mov_buffer_w16(&mov->io, 0);                /* Reserved */
    mov_buffer_w16(
        &mov->io, entry->data_reference_index); /* Data-reference index */

    mov_buffer_w16(&mov->io, 0); /* Reserved / Codec stream version */
    mov_buffer_w16(&mov->io, 0); /* Reserved / Codec stream revision (=0) */
    mov_buffer_w32(&mov->io, 0); /* Reserved */
    mov_buffer_w32(&mov->io, (int)(mov->track->fps * 100)); /* Reserved */
    mov_buffer_w32(&mov->io, mov->track->bitrate);          /* Reserved */
...
#ifdef _BILI_FIXED_
    if (mov->track->stsd.current->object_type_indication == MOV_OBJECT_DV_AVC ||
        mov->track->stsd.current->object_type_indication ==
            MOV_OBJECT_DV_HEVC) {
        snprintf(
            (char*)mov->compressorname, sizeof(mov->compressorname),
            "DOVI Coding");
    }
    mov_buffer_w8(&mov->io, strlen(mov->compressorname));
    mov_buffer_write(&mov->io, mov->compressorname, 31);
#else
    ...
    if (MOV_OBJECT_H264 == entry->object_type_indication)
        ...
        MOV_OBJECT_VP8 == entry->object_type_indication ||
        MOV_OBJECT_VP9 == entry->object_type_indication)
        size += mov_write_vpcc(mov);
    else if (MOV_OBJECT_DV_AVC == entry->object_type_indication) {
        size += mov_write_avcc(mov);
        // if (mov->v_extra_size)
        // {
        // 	mov_buffer_write(&mov->io, mov->v_extra_data, mov->v_extra_size);
        // 	size += mov->v_extra_size;
        // }
    } else if (MOV_OBJECT_DV_HEVC == entry->object_type_indication) {
        size += mov_write_hvcc(mov);
        // if (mov->v_extra_size)
        // {
        // 	mov_buffer_write(&mov->io, mov->v_extra_data, mov->v_extra_size);
...
static size_t mov_write_audio(
    ...
    // drm
    if (mov->is_drm) {
        mov_buffer_w32(&mov->io, MOV_ENCA);  // "enca"
    } else {
    ...
    else if (MOV_OBJECT_EAC3 == entry->object_type_indication) {
        size += mov_write_dec3(mov);
        // if (mov->a_extra_size)
        // {
        // 	mov_buffer_write(&mov->io, mov->a_extra_data, mov->a_extra_size);
        // 	size += mov->a_extra_size;
        // }
    } else if (MOV_OBJECT_AC3 == entry->object_type_indication) {
        size += mov_write_dac3(mov);
    }
#ifdef _BILI_FIXED_
    // 如果支持 MP3
    else if (
...
size_t mov_write_stsd(const struct mov_t* mov) {
    ...
    for (i = 0; i < track->stsd.entry_count; i++) {
        ...
        if (MOV_VIDEO == track->handler_type) {
            ...
            MOV_SUBT == track->handler_type ||
            MOV_TEXT == track->handler_type) {
