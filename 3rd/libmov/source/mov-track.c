...
struct mov_track_t* mov_add_track(struct mov_t* mov) {
    ...
    ptr = realloc(
        mov->tracks, sizeof(struct mov_track_t) * (mov->track_count + 1));
    if (NULL == ptr)
        return NULL;
...
void mov_free_track(struct mov_track_t* track) {
    size_t i;
#ifdef _BILI_FIXED_
    if (track->is_drm && track->sample_count && track->samples) {
        for (i = 0; i < track->sample_count; i++) {
            if (track->samples[i].subsample_count) {
                FREE(track->samples[i].senc_info);
            }
            track->samples[i].subsample_offset = 0;
            track->samples[i].subsample_count = 0;
        }
    }
    FREE(track->samples_buffer);
    track->samples_capacity = 0;
    track->samples_size = 0;
    track->sample_count = 0;
    track->sample_offset = 0;
    FREE(track->udta.data);
    track->udta.size = 0;
    track->udta.capacity = 0;
#else
    for (i = 0; i < track->sample_count; i++) {
...
#ifdef _BILI_FIXED_
        track->stsd.entries[i].extra_data = NULL;
#endif
    }
...
int mov_add_audio(
    struct mov_track_t* track, const struct mov_mvhd_t* mvhd,
    uint32_t timescale, uint8_t object, int channel_count, int bits_per_sample,
    ...
    track->tkhd.flags =
        MOV_TKHD_FLAG_TRACK_ENABLE | MOV_TKHD_FLAG_TRACK_IN_MOVIE;
...
int mov_add_video(
    struct mov_track_t* track, const struct mov_mvhd_t* mvhd,
    uint32_t timescale, uint8_t object, int width, int height,
...
int mov_add_subtitle(
    struct mov_track_t* track, const struct mov_mvhd_t* mvhd,
    uint32_t timescale, uint8_t object, const void* extra_data,
    ...
    subtitle->stream_type =
        MP4_STREAM_VISUAL;  // Visually composed tracks including video and text
                            // are layered using the 'layer' value.
...
// following order: Sample Description, Time to Sample, Sample to Chunk, Sample
// Size, Chunk Offset.
size_t mov_write_stbl(const struct mov_t* mov) {
    ...
    if (track->sample_count > 0 &&
        (count > 1 || track->samples[0].samples_per_chunk != 0))
