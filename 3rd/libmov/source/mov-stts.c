...
int mov_read_stts(struct mov_t* mov, const struct mov_box_t* box) {
    ...
    assert(
        0 == stbl->stts_count && NULL == stbl->stts);  // duplicated STTS atom
...
int mov_read_ctts(struct mov_t* mov, const struct mov_box_t* box) {
    ...
    assert(
        0 == stbl->ctts_count && NULL == stbl->ctts);  // duplicated CTTS atom
    ...
    for (i = 0; i < entry_count; i++) {
        ...
        stbl->ctts[i].sample_delta =
            mov_buffer_r32(&mov->io);  // parse at int32_t
    }
...
size_t mov_write_stts(const struct mov_t* mov, uint32_t count) {
    ...
    for (i = 0; i < track->sample_count; i++) {
        ...
        mov_buffer_w32(
            &mov->io, sample->samples_per_chunk);  // delta * timescale / 1000
    }
...
size_t mov_write_ctts(const struct mov_t* mov, uint32_t count) {
    ...
    mov_buffer_w8(
        &mov->io,
        (track->flags & MOV_TRACK_FLAG_CTTS_V1) ? 1 : 0); /* version */
    ...
    for (i = 0; i < track->sample_count; i++) {
        ...
        mov_buffer_w32(&mov->io, sample->first_chunk);  // count
        mov_buffer_w32(
            &mov->io, sample->samples_per_chunk);  // offset * timescale / 1000
    }
...
void mov_apply_stts(struct mov_track_t* track) {
    ...
    for (i = 0, n = 1; i < stbl->stts_count; i++) {
        for (j = 0; j < stbl->stts[i].sample_count; j++, n++) {
            track->samples[n].dts =
                track->samples[n - 1].dts + stbl->stts[i].sample_delta;
...
void mov_apply_ctts(struct mov_track_t* track) {
    ...
    for (i = 0; i < stbl->ctts_count; i++) {
        ...
        if (delta < 0 && dts_shift > delta &&
            delta != -1 /* see more cslg box*/)
            dts_shift = delta;
    }
    assert(dts_shift <= 0);
    ...
    for (i = 0, n = 0; i < stbl->ctts_count; i++) {
        for (j = 0; j < stbl->ctts[i].sample_count; j++, n++)
            track->samples[n].pts +=
                (int64_t)((int32_t)stbl->ctts[i].sample_delta - dts_shift);  // always as int, fixed mp4box delta version error
    }
