...
size_t mov_write_stco(const struct mov_t* mov, uint32_t count) {
    ...
    sample = track->sample_count > 0 ? &track->samples[track->sample_count - 1]
                                     : NULL;
    ...
    for (i = 0; i < track->sample_count; i++) {
        ...
        if (0 == co64)
            mov_buffer_w32(
                &mov->io, (uint32_t)(sample->offset + track->offset));
        else
    ...
    for (i = 0; i < track->sample_count; i++) {
        if (NULL != sample &&
            ...
            sample->sample_description_index ==
                track->samples[i].sample_description_index) {
...
void mov_apply_stco(struct mov_track_t* track) {
    ...
    stbl->stsc[stbl->stsc_count].first_chunk =
        stbl->stco_count + 1;  // fill stco count
    for (i = 0, n = 0; i < stbl->stsc_count; i++) {
        assert(stbl->stsc[i].first_chunk <= stbl->stco_count);
        for (j = stbl->stsc[i].first_chunk; j < stbl->stsc[i + 1].first_chunk;
             j++) {
            chunk_offset = stbl->stco[j - 1];  // chunk start from 1
            for (k = 0; k < stbl->stsc[i].samples_per_chunk; k++, n++) {
                track->samples[n].sample_description_index =
                    stbl->stsc[i].sample_description_index;
                ...
                assert(
                    0 == n || track->samples[n - 1].offset +
                                      track->samples[n - 1].bytes <=
                                  track->samples[n].offset);
            }
        }
    }
