...
struct mov_mvhd_t {
    ...
    uint32_t timescale;          // time-scale for the entire presentation, the number
                                 // of time units that pass in one second
...
struct mov_sample_entry_t {
    ...
    uint8_t object_type_indication;  // H.264/AAC MOV_OBJECT_XXX
...
struct dovi_config_t {
    uint8_t dv_version_major;
    uint8_t dv_version_minor;
    uint8_t dv_profile : 7;
    uint8_t dv_level : 6;
    uint8_t rpu_present_flag : 1;
    uint8_t el_present_flag : 1;
    uint8_t bl_present_flag : 1;
    // 3byte optional
    uint16_t dependecy_pid : 13;
    uint16_t reserved : 3;
    uint8_t dv_bl_signal_compatibility_id : 4;
    uint8_t reserved2 : 4;
};

struct eac3_config_t {
    uint8_t sample_rate_code;
    uint8_t surroud_mode;
...
// senc info
typedef struct {
    uint32_t bytes_clear_data;
    uint64_t bytes_protected_data;
} mov_senc_info_t;
