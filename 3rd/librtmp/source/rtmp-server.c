...
static int
rtmp_server_send_control(struct rtmp_t* rtmp, const uint8_t* payload, uint32_t bytes, uint32_t stream_id) {
...
static int
rtmp_server_send_onstatus_0(
    struct rtmp_server_t* ctx, double transaction, const char* level, const char* message, const char* description) {
    int r = (int)(rtmp_netstream_onstatus(ctx->payload, sizeof(ctx->payload), transaction, level, message, description)
                  - ctx->payload);
    return rtmp_server_send_control(&ctx->rtmp, ctx->payload, r, ctx->stream_id);
}

static int
rtmp_server_send_onstatus(
    ...
    return rtmp_server_send_onstatus_0(
        ctx, transaction, 0 == r ? RTMP_LEVEL_STATUS : RTMP_LEVEL_ERROR, 0 == r ? success : fail, description);
}

// handshake
static int
rtmp_server_send_handshake(struct rtmp_server_t* ctx) {
    int n, r;
...
static int
rtmp_server_send_set_chunk_size(struct rtmp_server_t* ctx) {
    int n, r;
...
static int
rtmp_server_send_acknowledgement(struct rtmp_server_t* ctx, size_t size) {
    int n, r;
...
static int
rtmp_server_send_server_bandwidth(struct rtmp_server_t* ctx) {
    int n, r;
...
static int
rtmp_server_send_client_bandwidth(struct rtmp_server_t* ctx) {
    int n, r;
    n = rtmp_set_peer_bandwidth(
        ctx->payload, sizeof(ctx->payload), ctx->rtmp.peer_bandwidth, RTMP_BANDWIDTH_LIMIT_DYNAMIC);
    r = ctx->handler.send(ctx->param, ctx->payload, n, NULL, 0);
    return n == r ? 0 : r;
}

static int
rtmp_server_send_stream_is_record(struct rtmp_server_t* ctx) {
    int n, r;
...
static int
rtmp_server_send_stream_begin(struct rtmp_server_t* ctx) {
    int n, r;
...
static int
rtmp_server_rtmp_sample_access(struct rtmp_server_t* ctx) {
    int n;
    struct rtmp_chunk_header_t header;
...
static void
rtmp_server_onabort(void* param, uint32_t chunk_stream_id) {
    struct rtmp_server_t* ctx;
...
static int
rtmp_server_onaudio(void* param, const uint8_t* data, size_t bytes, uint32_t timestamp) {
    struct rtmp_server_t* ctx;
    ctx = (struct rtmp_server_t*)param;
...
static int
rtmp_server_onvideo(void* param, const uint8_t* data, size_t bytes, uint32_t timestamp) {
    struct rtmp_server_t* ctx;
    ctx = (struct rtmp_server_t*)param;
...
static int
rtmp_server_onscript(void* param, const uint8_t* data, size_t bytes, uint32_t timestamp) {
    struct rtmp_server_t* ctx;
    ctx = (struct rtmp_server_t*)param;
...
// _result/_error
static int
rtmp_server_onconnect(void* param, int r, double transaction, const struct rtmp_connect_t* connect) {
    int n;
    struct rtmp_server_t* ctx;
    ctx = (struct rtmp_server_t*)param;
    ...
    if (0 == r) {
        n = (int)(rtmp_netconnection_connect_reply(
                      ctx->payload,
                      sizeof(ctx->payload),
                      transaction,
                      RTMP_FMSVER,
...
// _result/_error
static int
rtmp_server_oncreate_stream(void* param, int r, double transaction) {
    struct rtmp_server_t* ctx;
    ctx = (struct rtmp_server_t*)param;
    ...
    if (0 == r) {
        ...
        if (0 == r)
            r = (int)(rtmp_netconnection_create_stream_reply(
                          ctx->payload, sizeof(ctx->payload), transaction, ctx->stream_id)
                      - ctx->payload);
        else
            r = (int)(rtmp_netconnection_error(
                          ctx->payload,
                          sizeof(ctx->payload),
                          transaction,
...
static int
rtmp_server_ondelete_stream(void* param, int r, double transaction, double stream_id) {
    struct rtmp_server_t* ctx;
    ctx = (struct rtmp_server_t*)param;
    ...
    if (0 == r) {
        ...
        r = rtmp_server_send_onstatus(
            ctx, transaction, r, "NetStream.DeleteStream.Suceess", "NetStream.DeleteStream.Failed", "");
    }
...
static int
rtmp_server_onget_stream_length(void* param, int r, double transaction, const char* stream_name) {
    double duration = -1;
    struct rtmp_server_t* ctx;
    ctx = (struct rtmp_server_t*)param;
    ...
    if (0 == r && ctx->handler.ongetduration) {
        ...
        if (0 == r) {
            r = (int)(rtmp_netconnection_get_stream_length_reply(
                          ctx->payload, sizeof(ctx->payload), transaction, duration)
                      - ctx->payload);
...
static int
rtmp_server_onpublish(void* param, int r, double transaction, const char* stream_name, const char* stream_type) {
    struct rtmp_server_t* ctx;
    ctx = (struct rtmp_server_t*)param;

    if (0 == r) {
        r = ctx->handler.onpublish(ctx->param, ctx->info.app, stream_name, stream_type, ctx->info.tcUrl);
        if (0 == r) {
...
static int
rtmp_server_onplay(
    void* param, int r, double transaction, const char* stream_name, double start, double duration, uint8_t reset) {
    struct rtmp_server_t* ctx;
    ctx = (struct rtmp_server_t*)param;

    if (0 == r) {
        r = ctx->handler.onplay(ctx->param, ctx->info.app, stream_name, start, duration, reset, ctx->info.tcUrl);
        if (0 == r) {
            snprintf(ctx->stream_name, sizeof(ctx->stream_name) - 1, "%s", stream_name);
            snprintf(
                ctx->stream_type,
                sizeof(ctx->stream_type) - 1,
                "%s",
                -1 == start ? RTMP_STREAM_LIVE : RTMP_STREAM_RECORD);
            ...
            if (reset)
                r = 0 == r ? rtmp_server_send_onstatus(
                        ctx, transaction, 0, "NetStream.Play.Reset", "NetStream.Play.Failed", "")
                           : r;
        ...
        r = rtmp_server_send_onstatus(
            ctx, transaction, r, "NetStream.Play.Start", "NetStream.Play.Failed", "Start video on demand");
...
static int
rtmp_server_onpause(void* param, int r, double transaction, uint8_t pause, double milliSeconds) {
    struct rtmp_server_t* ctx;
    ctx = (struct rtmp_server_t*)param;
...
// failure:  _error message
static int
rtmp_server_onseek(void* param, int r, double transaction, double milliSeconds) {
    struct rtmp_server_t* ctx;
    ctx = (struct rtmp_server_t*)param;
...
static int
rtmp_server_onreceive_audio(void* param, int r, double transaction, uint8_t audio) {
    struct rtmp_server_t* ctx;
    ctx = (struct rtmp_server_t*)param;
...
static int
rtmp_server_onreceive_video(void* param, int r, double transaction, uint8_t video) {
    struct rtmp_server_t* ctx;
    ctx = (struct rtmp_server_t*)param;
...
static int
rtmp_server_send(
    void* param, const uint8_t* header, uint32_t headerBytes, const uint8_t* payload, uint32_t payloadBytes) {
    int r;
    struct rtmp_server_t* ctx;
    ctx = (struct rtmp_server_t*)param;
...
struct rtmp_server_t*
rtmp_server_create(void* param, const struct rtmp_server_handler_t* handler) {
    struct rtmp_server_t* ctx;
...
void
rtmp_server_destroy(struct rtmp_server_t* ctx) {
    size_t i;
    ...
    for (i = 0; i < N_CHUNK_STREAM; i++) {
        ...
        if (ctx->rtmp.in_packets[i].payload) free(ctx->rtmp.in_packets[i].payload);
    }
...
int
rtmp_server_getstate(struct rtmp_server_t* ctx) {
    return ctx->handshake_state;
}

int
rtmp_server_input(struct rtmp_server_t* ctx, const uint8_t* data, size_t bytes) {
    int r;
    size_t n;
    const uint8_t* p;
    ...
    while (bytes > 0) {
        switch (ctx->handshake_state) {
            ...
            case RTMP_HANDSHAKE_0: // C1: 4-time + 4-zero + 1528-random
                ...
                if (RTMP_HANDSHAKE_SIZE <= ctx->handshake_bytes) return -1;
...
int
rtmp_server_send_audio(struct rtmp_server_t* ctx, const void* data, size_t bytes, uint32_t timestamp) {
    struct rtmp_chunk_header_t header;
    if (0 == ctx->receiveAudio) return 0; // client don't want receive audio
...
int
rtmp_server_send_video(struct rtmp_server_t* ctx, const void* data, size_t bytes, uint32_t timestamp) {
    struct rtmp_chunk_header_t header;
    if (0 == ctx->receiveVideo) return 0; // client don't want receive video
...
int
rtmp_server_send_script(struct rtmp_server_t* ctx, const void* data, size_t bytes, uint32_t timestamp) {
    struct rtmp_chunk_header_t header;
...
int
rtmp_server_stop(rtmp_server_t* ctx) {
    return rtmp_server_send_onstatus_0(ctx, 0, RTMP_LEVEL_STATUS, "NetStream.Play.Stop", "");
}